...
##         9.12 Coagulation sink from an external user-provided subroutine
##         9.13 Coagulation sink given by one number
##         9.14 Coagulation sink for specific clusters from the command line
##         9.15 Printing out size-dependent sink in dry case
##         9.16 Hydrate average for size-dependent sink
##      9.2 Wall losses
##         9.21 Reading in wall losses for all the clusters from a given file
##         9.22 Wall loss parameterizations
##            9.221 Cloud 4 wall loss parameterization from the Excel file by Jasper Kirkby
##            9.222 Cloud 4 wall loss parameterization from Andreas Kürten
##            9.223 Cloud 4 wall loss parameterization used in the paper by Joao Almeida et al., 2013
##            9.224 Simplified Cloud 4 wall loss parameterization based on the paper by Andreas Kürten et al., 2015
##            9.225 Cloud 3 wall loss parameterization
##            9.226 Diffusion wall loss for a flow tube (originally for the tube of Hanson and Eisele, 2000)
##         9.23 Wall losses given by one number
##            9.231 Ift flow tube wall loss
##            9.232 Other size-independent wall loss
##         9.24 Wall losses for specific clusters from the command line
##         9.25 Printing out size-dependent wall losses in dry case
##         9.26 Hydrate average for size-dependent wall losses
##      9.3 Input sink from an external user-provided subroutine
##         9.31 Input sink for specific clusters from the command line
##
##   10. Collision rates
...
use Getopt::Long;	# Use command line input
use 5.10.1;         # Perl version
use List::Util qw(max min sum first); # Use some basic functions

my(@comments,@label,%num_from_label,%mol_num_from_name,$lsilent);
my($max_cluster,$string,$iclus,$jclus,$kclus);
my($ncols,$icol,$i,$j,$k,$l,$n,$ii,$jj,$kk,$ll,$jm,$km,$lm,$jc,$kc,$lc,$combined);
my($line,$line2,$max_terms_per_line);
my($recomb_coeff,$mass1,$mass2,$vol1,$vol2,$lmonomer1,$lmonomer2);
...
my($temp_val,$temp_val2,$temp_val3,$size_array,@delta_g,@l_no_delta_g_data,$boltz,$Na,$kcal_per_mol_to_J,@evap_coef,$coll_coef_temp);
...
my($exp_loss_coefficient,$exp_loss_exponent,$exp_loss_ref_cluster,$exp_loss_ref_size,$default_monomer,$lfound_several,$lfound_default);
...
my(@ind_quad_loss,@ind_quad_form,@ind_quad_loss_extra,@ind_quad_form_extra, @ind_lin_loss,@ind_lin_form, %l_quad_bound,$l_coll_factor_mcmc_all,$l_coll_factor_mcmc_n_n,$l_coll_factor_mcmc_n_i,$l_coll_factor_mcmc_n_g,$l_coll_factor_mcmc_rec,$l_coll_factor_mcmc_any,$l_q_neg_mcmc,$l_q_pos_mcmc,$l_wl_mcmc,@wl_mcmc_only,$l_mcmc_any,$n_mcmc_coefs, $missing_energies,$missing_fcr,$l_mcmc_coef,@l_special_mcmc_wl,$icoef_wl);
my(@cs_only,@cs_only_hydr,$one_cs_for_all,$one_cs_for_almost_all,@wl_only,@wl_only_hydr,$one_wl_for_all,$one_wl_for_almost_all,@insink_only);
my($icoef_coll,@l_coll_not_used,$nmolecules,@molecules);
my($lhydrate_average,$rh,$pw,$psw,$name_water_def,$name_water,$mass_water,$dens_water,$volume_water,$iwater,$nwaters,$lfound_hydr,$ihydr,$jhydr,$khydr,$nhydr,@lhas_hydr,@delta_g_hydr,@delta_g_temp, @distr,@distr_temp,@distr_temp2,@distr_temp3,$sum,$sum2);
...
my($dil_value,$dil_str,$use_coag_sinks,$use_wall_terms,$use_dilution,$use_input_sink,$include_generic_ions,$no_generic_ions,$include_generic_neg,$include_generic_pos,$no_generic_neg,$no_generic_pos);
my($module_insink,$routine_insink);
my($variable_temp,$variable_cs,$variable_ion_source,$l_variable_parameters,$n_variable_parameters,$cs_value,$cs_value_in,$no_ambient_params_in_syst);
my($tag,$append_to_file_names,$append_to_eq,$append_to_syst,$append_to_subroutine_names,$append_to_eq_names,$append_to_syst_names,$append_to_ext_names,$label_generic_neg, $label_generic_pos,$label_proton,$label_missing_proton,@coef_true);
my(@negative_clusters,@positive_clusters);
my($stopping);
my($iclus_label,$jclus_label,$kclus_label,$disable_useless_collisions,$keep_useless_collisions,$coef_temp,$include_ion_terms,$line_temp,$fcr_temp);
...
my($str_temp,$str_temp_form,$str_temp2,$str_temp3,$str_temp4,$value_string,$coll_string,$hydr_string,$evap_string,$fixed_string,$fcr_string);
...
my(@label_out,@clust_comp_out,$n_comp_out,$n_out_clust,@range_out_clust,@ind_out_clust);
my($l_old_output,$ref,$comp_ok);
my(@clust_comp,@clust_mass,@clust_radius,@clust_volume);
my(@l_can_be_lost_bound,@l_can_evaporate);
my($clust_mass_max,$diameter_max,$mob_diameter_max,$n_at_bound,@clus_at_bound);
my($lloop,$lloop_cs,$lloop_restrict,$lloop_boundary,$nmol_types_used,@nmol_ranges_array,$nmol_ranges_list,$nmol_comp_list,$nmol_ranges_ind,$nmol_ranges_ij_ind,$cluster_from_indices,@cluster_from_indices_array,$monomer_indices,$nmol_cs_list,@source_function,@source_function_clust,@mol_types_used,@mol_names_used,$loop_coll_coef,$loop_evap_coef,$rlim_no_evap,$rlim_no_evap_str,$lloop_j_frac,$lloop_j_tests,$lgrowth_for_j_tests,$c_growth_compound,$c_growth_compound_str,$rlim_growth_compound_str,$lhs_tests);
...
# All variables beginning with l are treated as logical variables: 1 is true, 0 is false
...
$ldo_table_out = 1;
$l_save_outgoing = 0;
$l_save_outgoing_clust = 0;
$l_save_coag_per_clust = 0;
$disable_nonmonomers = 0;
@nonmonomer_collisions_only = ();
$enable_rec_nonmonomers = 0;
$disable_evap = 0;
$disable_nonmonomer_evaps = 0;
...
$lloop_restrict = 0;
$lloop_boundary = 0;
$nclust_included = 'nclust';
$loop_coll_coef = 'hard_spheres';
$loop_evap_coef = 'Kelvin';
$rlim_no_evap = '';
$lloop_j_frac = 0;
$l_jlim = 0;
$jlim_sizes_list = '';
...
$append_to_eq_names = '';
$append_to_syst_names = '';
$append_to_ext_names = '';
$tag = '';
$no_ambient_params_in_syst = 0;
...
$input_file_name = 'input_def.inp';
$free_energy_file_name = 'G_def.txt';
$dip_file_name = 'dip_pol_def.txt';
...
$cs_value_in = '';
$cs_value = "1e-3";
$coag_terms = 'exp_loss';			# Options: exp_loss, bg_loss, number in 1/s, external
...
$wl_coef_file_name = '';
$wall_terms = 'CLOUD4_JA';			# Options: cloud4_JA, cloud4_JK, cloud4_AK, cloud4_simple, cloud3, ift, diffusion, number in 1/s
$tube_radius_in = '';
$tube_pressure_in = '';

$dil_value = 9.6e-5;				# Dilution from CLOUD: dil = flow / vol / 60000

$insink_terms_in = '';              # User-defined input sink
$insink_terms = '';
$module_insink = '';
$routine_insink = '';
...
$l_variable_parameters = 0;
$n_variable_parameters = 0;
$variable_temp = 0;
$variable_cs = 0;
$variable_ion_source = 0;

# Variables related to hydration / equilibrium with respect to a given compound
# Note that while water is the principle compound assumed to be equilibrated at atmospheric conditions, also
# some other compound can be assumed to be in equilibrium by giving as input its name, mass, density and partial pressure
$rh = 0;
$pw = 0;
$name_water=$name_water_def;        # Name used to identify hydrates in the input files for cluster properties
$mass_water=18.02;                  # amu
...
GetOptions('help'=>\$help, 'silent'=>\$lsilent, 'fortran'=>\$lfortran, 'all_output|a!'=>\$l_all_output, 'old_output_order|old_output'=>\$l_old_output, 
...
'disable_nonmonomer_evaps|no_nonmonomer_evaps|me'=>\$disable_nonmonomer_evaps, 'all_collisions|keep_useless_collisions'=>\$keep_useless_collisions, 'disable_boundary'=>\$disable_boundary, 
'print_boundary'=>\$l_print_boundary, 'keep_boundary_clusters'=>\$keep_boundary_clusters, 'print_3d_flux|flux_3d' => \$print_3d_flux, 
'disable_excluded'=>\$disable_excluded, 'disable_excluded_ionization'=>\$disable_excluded_ionization, 
'save_outgoing'=>\$l_save_outgoing, 'save_outgoing_clust'=>\$l_save_outgoing_clust, 'save_coag_per_clust'=>\$l_save_coag_per_clust, 
'no_derivative_clus|no_eq=s' => \@no_derivative_clus, 'increase_time'=>\$l_increase_time, 
'loop' => \$lloop, 'loop_cs' => \$lloop_cs, 'loop_restrict|restrict' => \$lloop_restrict, 'loop_boundary|boundary' => \$lloop_boundary, 
'loop_coll_coef=s' => \$loop_coll_coef, 'loop_evap_coef=s' => \$loop_evap_coef, 'rlim_no_evap|evaplim=f' => \$rlim_no_evap, 
...
'append_to_file_names|append=s'=>\$append_to_file_names, 'append_to_eq=s'=>\$append_to_eq, 'append_to_syst=s'=>\$append_to_syst,
'tag=s'=>\$tag, 'no_ambient_params_in_syst|no_ambient'=>\$no_ambient_params_in_syst,
'append_to_subroutine_names|append_to_subroutines=s'=>\$append_to_subroutine_names,
'append_to_eq_names=s'=>\$append_to_eq_names, 'append_to_syst_names=s'=>\$append_to_syst_names, 'append_to_ext_names=s'=>\$append_to_ext_names,
...
'wl_mcmc' => \$l_wl_mcmc, 'wl_mcmc_only=s' => \@wl_mcmc_only, 'evap_coef_mcmc' => \$l_evap_factor_mcmc, 
...
'dip_file_name|dip=s' => \$dip_file_name, 'radius_file_name|rad=s' => \$radius_file_name, 
'rh=f' => \$rh, 'pw|peq=f' => \$pw, 'name_water|name_eq=s' => \$name_water, 'mass_water|mass_eq=f' => \$mass_water, 'density_water|density_eq=f' => \$dens_water, 
...
'disable_dilution'=>\$disable_dilution, 'disable_input_sink'=>\$disable_input_sink, 
'use_coag_sinks|use_coag|use_cs'=>\$use_coag_sinks, 'use_wall_terms|use_wall_losses|use_wl'=>\$use_wall_terms, 
'use_dilution|use_dil'=>\$use_dilution, 'use_input_sink|use_insink'=>\$use_input_sink, 
'coag_terms|cs=s'=>\$coag_terms_in, 'cs_value=f'=>\$cs_value_in, 
'exp_loss_coefficient=f' => \$exp_loss_coefficient, 'exp_loss_exponent=f' => \$exp_loss_exponent, 
'exp_loss_ref_cluster=s' => \$exp_loss_ref_cluster, 'exp_loss_ref_size=f' => \$exp_loss_ref_size, 
...
'flowtube_radius=f' => \$tube_radius_in, 'flowtube_pressure=f' => \$tube_pressure_in, 
'dil_value=f' => \$dil_value, 
'insink_terms|insink=s'=>\$insink_terms_in, 
'cs_only=s' => \@cs_only, 'wl_only=s' => \@wl_only, 'insink_only=s' => \@insink_only, 
...
if($help){
		die "Usage:\tacdc.pl [--options]\n\n
        See the ACDC manual for technical information. Only the features that are not (yet) added to the current manual version are listed below.\n\n
		MCMC options:\n
		\t--evap_coef_mcmc
		...
		if(!$lsilent){
            print "\nSee ACDC manual at https://github.com/tolenius/ACDC for help.\n\n";
        }
}
...
if($variable_cs && $#cs_only>=0){
	for($icol=0;$icol<=$#cs_only;$icol++){
		@columns=split(',',$cs_only[$icol]);
		if($columns[1] ne 0){
			die "\nCannot define fixed coagulation sink values for specific clusters when using a varying sink.\n\n";
		}
	}
}
...
$str_temp = "Not using coagulation sinks.\n";
$str_temp2 = "Not using wall losses.\n";
$str_temp3 = "Not using dilution.\n";
$str_temp4 = "Not using additional user-defined sinks.\n";
if(($use_coag_sinks+$use_wall_terms+$use_dilution+$use_input_sink+$variable_cs+$l_wl_mcmc)>0 || $coag_terms_in ne '' || $wall_terms_in ne '' || $insink_terms_in ne '' || $#cs_only>=0 || $#wl_only>=0 || $#insink_only>=0 || $#wl_mcmc_only>=0){
	if(($disable_coag_sinks+$disable_wall_terms+$disable_dilution+$disable_input_sink)>0){
		die "\nSpecify which losses to use or which losses to disable, but not both!\n\n";
	}
	if($use_coag_sinks==0 && $variable_cs==0 && $coag_terms_in eq '' && $#cs_only<0){
		$disable_coag_sinks = 1;
	}elsif($coag_terms_in eq '' && $#cs_only>=0){
		$coag_terms_in = '0.0';
		$str_temp = "Using coagulation sinks (but only for some clusters).\n";
	}else{
		$str_temp = "Using coagulation sinks.\n";
	}
	if($use_wall_terms==0 && $l_wl_mcmc==0 && $#wl_mcmc_only<0 && $wall_terms_in eq '' && $#wl_only<0){
		...
		$wall_terms_in = '0.0';
		$str_temp2 = "Using wall losses (but only for some clusters).\n";
	}elsif($#wl_only>=0 && ($l_wl_mcmc || $#wl_mcmc_only>=0)){
		die "wl_only cannot be used together with mcmc wall losses";
	}else{
		$str_temp2 = "Using wall losses.\n";
	}
	if($use_dilution==0){
		$disable_dilution = 1;
	}else{
		$str_temp3 = "Using dilution.\n";
	}
    if($use_input_sink==0 && $insink_terms_in eq '' && $#insink_only<0){
		$disable_input_sink = 1;
    }elsif($insink_terms_in eq '' && $#insink_only>=0){
        die "\nFor now, the user-defined sink must be a subroutine; only single values for some clusters cannot be given\n\n";
    }elsif(!$lfortran){
        die "\nThe user-defined sink option works only with the Fortran version, for now.\n\n";
    }else{
		$str_temp4 = "Using additional user-defined sinks.\n";
	}
}elsif(($disable_coag_sinks+$disable_wall_terms+$disable_dilution+$disable_input_sink)==0){
	$disable_coag_sinks = 1;
	$disable_wall_terms = 1;
	$disable_dilution = 1;
    $disable_input_sink = 1;
}else{
	if($disable_coag_sinks==0){
		$str_temp = "Using coagulation sinks.\n";
	}
	if($disable_wall_terms==0){
		$str_temp2 = "Using wall losses.\n";
	}
	if($disable_dilution==0){
		$str_temp3 = "Using dilution.\n";
	}
    if($disable_input_sink==0){
		$str_temp4 = "Using additional user-defined sinks.\n";
	}
}
if(!$lsilent){
...
if($insink_terms_in ne ''){
	$insink_terms = $insink_terms_in;
}
if(&is_pos_number($coag_terms)){
	if($#cs_only>=0){
		$one_cs_for_all = 0;
		$one_cs_for_almost_all = 1;
	}else{
		$one_cs_for_all = 1;
		$one_cs_for_almost_all = 0;
	}
}else{
	$one_cs_for_all = 0;
	$one_cs_for_almost_all = 0;
...
if((!$disable_coag_sinks && (!$one_cs_for_all || !$variable_cs)) || !$disable_wall_terms || ($lfortran && !$disable_dilution) || !$disable_input_sink){
	$l_use_get_losses = 1;
}else{
	$l_use_get_losses = 0;
}
...
if(!$disable_coag_sinks){
    if($coag_terms =~ m/^exp_loss$/i && $exp_loss_ref_size ne ''){
        $exp_loss_ref_size *= 0.5*1e-9; # diameter -> radius, nm -> m
...
if(!$disable_input_sink){
    # Input for the user-defined sink is either "$routine_insink" or "$routine_insink,$module_insink"
    @temp_array=split(',',$insink_terms);
    $routine_insink = $temp_array[0];
    if($#temp_array>0){
        $module_insink = $temp_array[1];
    }
}
...
if($lloop_cs || $lloop_restrict || $lloop_boundary){
	$lloop = 1;
}
if($lloop_boundary){
	$lloop_restrict = 1;
...
if($lloop_restrict){
	$nclust_included = 'nclust_inc';
}
# Disabling cluster fissions if the Kelvin evaporation is used in the loop mode
if($lloop && $loop_evap_coef =~ m/^Kelvin$/i && (!$disable_evap || !$disable_nonmonomer_evaps)){
	$disable_nonmonomer_evaps = 1;
    if(!$lsilent){
...
### Determining if outgoing flux per outgrown cluster is tracked ###
# (works only for the Fortran version)
if($l_save_outgoing_clust){
    $l_save_outgoing = 1;
    if(!$lfortran){
        die "\nSaving the concentration of outgoing flux for each outgrown cluster is a feature of the Fortran version\n";
    }
}

### Determining if coagulation losses per cluster are tracked ###
# (works only for the Fortran version)
if($l_save_coag_per_clust){
    if(!$lfortran){
        die "\nSaving the concentration lost by coagulation scavenging for each cluster is a feature of the Fortran version\n";
    }
    if(!$l_save_outgoing){
        $l_save_outgoing = 1; # Doesn't make much sense to include coag. per cluster but no "dynamic" formation rate
        print "\nTurning on the option to save the outgoing concentrations, as also the option to save coagulation for each cluster is on\n";
        print "(Cannot see why they would not be used together)\n";
    }
}
...
if($append_to_file_names ne ''){
	if($lfortran){
        if($append_to_eq ne '' || $append_to_syst ne ''){
            die "\nGive file name suffices either for all output files, or separately for different Fortran files, but not both!\n\n";
        }
        $append_to_eq=$append_to_file_names;
        $append_to_syst=$append_to_file_names;
	}else{
...
if($append_to_eq ne ''){
    $output_file_name_fortran =~ s/\.f90$/$append_to_eq\.f90/;
}
if($append_to_syst ne ''){
    $system_file_name_fortran =~ s/\.f90$/$append_to_syst\.f90/;
}
...
### Possible suffices in subroutine names ###
if($append_to_subroutine_names ne ''){
    if($append_to_eq_names ne '' || $append_to_syst_names ne '' || $append_to_ext_names ne ''){
        die "\nGive subroutine name suffices either for all subroutines, or separately for subroutines in different Fortran files, but not both!\n\n";
    }
    $append_to_eq_names=$append_to_subroutine_names;
    $append_to_syst_names=$append_to_subroutine_names;
    $append_to_ext_names=$append_to_subroutine_names;
}

### Tag for labeling the simulation system and/or set-up ###
if($tag ne ''){
    if(!$lfortran){
        die "\nThe tag option is only for Fortran (for easily recognizing the system during run) - for Matlab, you might want to use file name suffices\n\n";
    }
    $tag = "'$tag'";
}
...
if($variable_temp || $variable_cs || $variable_ion_source){
	$l_variable_parameters = 1;
	$n_variable_parameters = $variable_temp+$variable_cs+2*$variable_ion_source;
}
if($variable_cs){
	if(!$lfortran){
		die "\nThe --variable_cs option works only with the Fortran version.\n\n";
	}
}
if($variable_ion_source){
	if(!$lfortran || $lloop){
		die "\nThe --variable_ion_source option works only with the Fortran version in the small set mode.\n\n";
	}
...
if($rh > 0 || $pw > 0){
    if($rh > 0 && $pw > 0){
        die "\nUse either --rh (in %) or --pw (in Pa) for water vapor concentration to consider effects of hydration, but not both\n\n";
    }
	if($variable_temp==1){
...
if(($disable_evap && !$lhydrate_average) && !$variable_temp){
...
# Properties of water for hydrate distribution calculations
$mass_water=$mass_water*$mass_conv; # amu -> kg
...
$psw = exp(-2991.2729*$temperature**(-2)-6017.0128/$temperature+18.87643854-0.028354721*$temperature+0.17838301e-4*$temperature**2-0.84150417e-9*$temperature**3+0.44412543e-12*$temperature**4+2.858487*log($temperature)); # Pa
# Previously used formula from Hanna Vehkamäki: Classical Nucleation Theory in Multicomponent Systems, DOI 10.1007/b138396, Springer, 2006
#$psw = exp(77.344913-7235.4247/$temperature-8.2*log($temperature)+0.0057113*$temperature);

if($rh > 0){
    $pw = $rh/100*$psw;
    if(!$lsilent && ($name_water ne $name_water_def)){
        print "\nAssuming the saturation vapor pressure of water for compound $name_water;\n";
        print "if this is not correct, give the concentration of $name_water as absolute vapor pressure and not as RH\n\n";
    }
}elsif($pw > 0){
    $rh = $pw/$psw*100;
    if(!$lsilent && ($name_water ne $name_water_def)){
        print "\nRH information obtained for compound $name_water assuming the saturation vapor pressure of water;\n";
        print "(RH is only printed out for information, and only the absolute vapor pressure is used in calculations)\n\n";
    }
}
...
# Number in the cluster composition array for proton (positive hydrogen ion)
$proton=-1;
# Number in the cluster composition array for missing_proton (removed from an acid to charge it negatively)
$missing_proton=-1;
# Number of actual molecule types (monomers) including both neutral and charged molecules (proton/missing proton not counted as a molecule)
$nmolecules = 0;
# Number of charging states
$ncharges = 1; # neutral state always exists
	...
	if($#columns < $nmol_types-1){
		...
		# the charges of all the molecules
			...
			for($imol=0;$imol<$nmol_types;$imol++){
				...
				# Default values for corresponding molecules and ions
                $corr_neutral_mol[$imol] = '';
                $corr_negative_mol[$imol] = '';
                $corr_positive_mol[$imol] = '';
                $n_corr_neutral_mol[$imol] = -1;
                $n_corr_negative_mol[$imol] = -1;
                $n_corr_positive_mol[$imol] = -1;
...
for($imol=0;$imol<$nmol_types;$imol++){
	...
	if($charge[$imol] == 0){
		...
		if($acid_strength[$imol] > 0 || $base_strength[$imol] > 0){
...
if($variable_ion_source && (!$include_negative_ion_terms || !$include_positive_ion_terms)){
	die "\nThe --variable_ion_source option works only when ions are included.\n\n";
}
...
################################################################################
	...
	# $max_cluster is the largest number for a proper cluster (possibly including the formed particles)
	# $max_cluster_number is the largest number for a cluster or generic ion
	$max_cluster_number=$max_cluster;
	$n_generic_neg=-1;
	$n_generic_pos=-1;
...
################################################################################
	...
	$i = $i + 1;
    $n_flux_insink=$max_cluster_number+$i;
	$i = $i + 1;
    ...
    $comments[$n_flux_insink]="% $n_flux_insink is for user-defined losses";
    ...
    # Ensure that all labels are included in num_from_label
    for($iclus=1;$iclus<=$max_n_flux;$iclus++){
        if(! exists $num_from_label{"$label[$iclus]"}){
            $num_from_label{"$label[$iclus]"} = $iclus;
        }
    }
    ...
    # also num_from_label will be updated accordingly
    ...
    # Labels for outgrown products; the 0th element is empty in order to number them from 1 similarly to the simulated clusters
    # This will be updated when we find out the outgrown clusters while looping through the collisions
    @label_out = ('');
    @clust_comp_out = ([0]);
    $n_comp_out = 0;
...
# Number of equations to allocate the Fortran index and coefficient arrays
$size_ind_coef = $max_n_flux;

# Numbers of optional elements in addition to the cluster and flux names
$n_coag_per_clust = 0;
$n_out_clust = 0;
@range_coag_per_clust = (-1, -1);
@range_out_clust = (-1, -1);

if($l_save_outgoing){
    # If also individual outgrowing clusters are saved, their number will be added after it has been determined from the equations
	$size_C_out = $max_n_flux;
    if($l_save_coag_per_clust){
        # Coagulation per cluster
        # Note: generic ions are included here; they can be excluded by using $max_cluster instead of $max_cluster_number, but
        # in this case also the coagulation arrays in all Fortran routines that apply ACDC must be allocated accordingly
        $n_coag_per_clust = $max_cluster_number;
        $size_C_out += $n_coag_per_clust;
    }
}else{
...
if($lloop){
	...
	for($imol=0;$imol<$nmol_types;$imol++){
		if($n_max_type[$imol]>0){
			...
			$nmol_comp_list .= "0:".(2*$n_max_type[$imol]).",";
			$nmol_ranges_ind .= "i".($nmol_types_used).", ";
			$nmol_ranges_ij_ind .= "ij_ind(".($nmol_types_used)."),";
		}
	}
	$nmol_ranges_list =~ s/,$//;
	$nmol_comp_list =~ s/,$//;
	$nmol_ranges_ind =~ s/, $//;
	$nmol_ranges_ij_ind =~ s/,$//;
	if($lloop_restrict && $nmol_types_used == 1){
		die "Found only one molecule type in the input file; options --restrict and --boundary are for several molecule types\n";
	}
	if($l_j_in && $nmol_types_used > 1){
		die "Found $nmol_types_used molecule types in the input file; option --j_in only works with one\n";
	}
	
	$nmolecules = $nmol_types_used; # loop mode: monomers = molecules, as there are no charges included for now
	if($nmol_types_used==1){
		$monomer_indices = "1";
	}else{
		$monomer_indices = "n_monomers";
	}
	
	# Determine the index label for the outgoing flux, or molecule numbers for the cs cluster
	if($lloop_cs){
		if($nmol_types_used==1){
			$n_flux_out = 'n_cs';
		}elsif(defined $cs_cluster[0]){
			for($imol=0;$imol<$nmol_types;$imol++){
				if($cs_cluster[$imol]>$n_max_type[$imol]){
					die "In the loop mode, the coagulation sink cluster must be inside the system\n";
				}
			}
			$nmol_cs_list = join(',',@cs_cluster[@mol_types_used[1..$nmol_types_used]]);
		}else{
			# If no cs cluster is given, use the cluster with the largest number of molecules
			$nmol_cs_list = $nmol_ranges_list;
		}
	}else{
		$n_flux_out = "$nclust_included+1";
	}
...
if(!$lsilent){
    print "\nMolecule types included in the system:\n";
    for($imol=1;$imol<=$nmol_types_used;$imol++){
        print "name $mol_names_used[$imol], charge $charge[$mol_types_used[$imol]]\n";
    }
    print "\n";
}
...
if($nonstandard_reaction_file_name eq ''){
	...
	for($iline=0;$iline<$number_of_lines;$iline++){
		...
		for($icol=2;$icol<$#columns;$icol+=2){
			...
			$coef_temp = sprintf("%.4f",$columns[$icol]);
			if($lfortran){
				$coef_temp =~ s/e/d/;
			}
...
#print "$temp_label: mass = $clust_mass[$iclus][$nwaters], radius =  $clust_radius[$iclus][$nwaters]\n";
	...
	for($iclus=1;$iclus<=$max_cluster;$iclus++){
		...
		if(!(exists $delta_g[$iclus])){
            ...
            }elsif($lhydrate_average){
                if($lhas_hydr[$iclus]==0){
                    # Evaporation rate might still be given in an input file, but flag the cluster until the file is read
                    $l_no_delta_g_data[$iclus]=1;
                }
            }else{
                $l_no_delta_g_data[$iclus]=1;
            }
            if($l_no_delta_g_data[$iclus]){
                $string1 .= "Missing energy data for cluster $label[$iclus]";
                if($lhydrate_average){
                    $string1 .= " and its hydrates";
                }
                $string1 .= " in the energy file $free_energy_file_name.\n";
                if($evaporation_coef_file_name eq '' && $disable_evap_only[$iclus]==0){
                    $stopping = 1;
                }
            }
		}
		...
		if($lhas_hydr[$iclus]==1){
			...
			for($ihydr=0;$ihydr<=$#delta_g_temp;$ihydr++){
				if(defined $delta_g_temp[$ihydr]){
					...
					$distr_temp[$ihydr] = ($pw/$pressure)**$ihydr*exp(-$delta_g_temp[$ihydr]/$boltz/$temperature);
					$sum += $distr_temp[$ihydr];
                    #print "$label[$iclus]: hydrate with $ihydr waters included\n";
				}else{
					print "Note: Energy for the hydrate of $label[$iclus] with $ihydr waters is not given, although the cluster has hydrates with more than $ihydr waters.\n";
				}
			}
		...
		for($jclus=$iclus;$jclus<=$max_cluster_number;$jclus++){
			...
			# Otherwise just collide the clusters and see what we get:
				...
				if($valid_coll && !$valid){
					...
					$valid_evap=0;
					# Skip the collision, if it doesn't lead out of the system and fancier boundary conditions are disabled
					if($disable_boundary && $lout==0){
						$valid_coll=0;
					}
					...
					if($lout>0){
                        ...
                        # See if this outgrowing cluster is already listed
                        # If not, create a new element
                        if(!($str_temp ~~ @label_out)){
                            push(@label_out,$str_temp);
                            ($string1,$string2) = &determine_cluster_composition($str_temp);
                            @temp_array=@$string1;
                            push(@clust_comp_out,[@temp_array]);
                            $n_comp_out += 1;
                        }
                        $i = first { $label_out[$_] eq $str_temp } 0..$#label_out;
                        $ind_out_clust[$iclus][$jclus] = $i;
                        $ind_out_clust[$jclus][$iclus] = $i;
					...
					if(!$valid && $lout==0 && $kclus!=$n_flux_rec){
						...
						if($keep_boundary_clusters){
							...
							if($str_temp ~~ @label_inc_boundary_clusters){
                                ...
                                $num_from_label{"$label_inc_boundary_clusters[$kclus_bound]"} = $kclus_bound;
							}						
...
###########################
		...
		if(!$disable_input_sink){
			$coef_lin{"$n_flux_insink,$n_flux_insink,$iclus"} = "insink($iclus)";
			$ind_lin_loss[$iclus][0] += 1;
			push(@{ $ind_lin_loss[$iclus] },$n_flux_insink);
			push(@{ $ind_lin_loss[$iclus] },$n_flux_insink);
			$ind_lin_form[$n_flux_insink][0] += 1;
			push(@{ $ind_lin_form[$n_flux_insink] },$n_flux_insink);
			push(@{ $ind_lin_form[$n_flux_insink] },$iclus);
		}
...
## All terms in the derivative / concentration array are now collected, save the final array size

if($l_save_outgoing_clust){
    $n_out_clust = $n_comp_out;
    $size_C_out += $n_out_clust;
}

# Allocation of isconst, source and fitted, obtained from sources_and_constants
# Problem with the "save" attribute - the size can't be an input parameter in the current implementation
#$size_C_eqn = 'neqn';
$size_C_eqn = $size_C_out;

# Indices of the saved coagulated and outgrown clusters
# Note that these are special cases, since they are NOT explicitly included in the equation terms, but are instead
# deduced from the overall coagulation and outgrowth terms
# Therefore, these are always the last, additional elements in the derivative / concentration array
$i = $max_n_flux;
if($l_save_coag_per_clust){
    $j = $n_coag_per_clust;
    @range_coag_per_clust = ($i+1, $i+$j);
    $i += $j;
}
if($l_save_outgoing_clust){
    $j = $n_out_clust;
    @range_out_clust = ($i+1, $i+$j);
    $i += $j;
}
...
if($#source_function>=0){
	$lines{'source'} = "\t% time dependent source terms defined by functions\n";
	if($lfortran){
		$lines{'source'} =~ s/%/!/;
		$lines{'source_def'} = "\treal(kind(1.d0)), external :: ";
	}
	$ii=1;
	for($i=0;$i<=$#source_function;$i++){	
		...
		if($source_function_clust[$i] !~ /^n_j_in$/){
			...
			if(!$lloop){
				...
				$iclus = "clust_from_indices(".join(',',@temp_array[@mol_types_used[1..$nmol_types_used]]).")";
			}else{
				$imol=1;
...
if($l_reac_in_clus){
	...
	REACTIONLINES: for($iline=0;$iline<$number_of_lines;$iline++){
		...
		for($icol=2;$icol<=$n_react+1;$icol++){
			if($lhydrate_average && $columns[$icol] eq $name_water){
				$n_reacting_W += 1;
				next;
		...
		for($icol=$n_react+$n_prod+4;$icol<=$#columns;$icol++){
			if($columns[$icol] =~ /^!/){
				...
				}elsif(($columns[$icol] eq $name_water) && $lhydrate_average && ($pw>0)){
					$l_forbidden_W = 1;
				}
			}else{
		...
		for($iclus=1;$iclus<=$max_cluster;$iclus++){
			...
			DIRECTIONS: for($i=1;$i<=2;$i++){
				...
				# checking that it doesn't contain the forbidden molecules (other than possibly hydration water)
				...
				if(exists $coef_reac{"$iclus,$jclus"}){
					...
					if($lhydrate_average){
						...
						for($ihydr=0;$ihydr<=$#{ $distr[$iclus] };$ihydr++){
							...
							$string1 .= sprintf("%.4f",$distr[$iclus][$ihydr])."*".$coef_reac{"$iclus,$jclus,$ihydr"}."+";
						}
						$string1 =~ s/\+$//;
						for($jhydr=0;$jhydr<=$#{ $distr[$jclus] };$jhydr++){
							$coef_reac{"$jclus,$iclus"} += $distr[$jclus][$jhydr]*$coef_reac{"$jclus,$iclus,$jhydr"};
							$string2 .= sprintf("%.4f",$distr[$jclus][$jhydr])."*".$coef_reac{"$jclus,$iclus,$jhydr"}."+";
						}
						$string2 =~ s/\+$//;
					}
...
if($lfortran){
	...
	print SYST "module acdc_system$append_to_syst_names\n\nimplicit none\n\n";
    if($tag !~ /^\s*$/){
        $str_temp = $tag;
    }else{
        $str_temp = "''";
    }
    print SYST "character(len=*), parameter :: tag = $str_temp\t\t\t! tag for the simulation system\n\n";
	if(!$lloop){
        print SYST "logical, parameter :: small_set_mode = .true.\t\t\t! small set of clusters (default mode)\n\n";
		...
		print SYST "integer, parameter :: nclust_max = $max_cluster_number, neq_max = $size_C_out\t! same as nclust and neq in the small set mode\n";
	}else{
        print SYST "logical, parameter :: small_set_mode = .false.\t\t\t\t\t\t\t! very large set of clusters (loop mode)\n\n";
		# Name the system size parameters in a different way in the module to avoid confusion and errors when allocating arrays in drivers etc.,
		# as the loop system may be restricted
		if($lloop_restrict){
			print SYST "integer, parameter :: nclust = -1, neq = -1\t\t\t\t\t\t! not pre-determined in the restrict mode\n";
			print SYST "logical, parameter :: restrict_system = .true.\t\t\t\t\t\t\t! exclude given cluster compositions\n";
		}else{
			print SYST "integer, parameter :: nclust = $max_cluster_number, neq = $size_C_out\n";
			print SYST "logical, parameter :: restrict_system = .false.\t\t\t\t\t\t\t! all cluster compositions allowed\n";
		}
        print SYST "integer, parameter :: nclust_max = $max_cluster_number\t\t\t\t\t\t! max. number of clusters, molecules and ions\n";
		print SYST "integer, parameter :: neq_max = $size_C_out\t\t\t\t\t\t\t! max. number of equations\n";
	}
	if($l_jlim){
		print SYST "\ninteger, parameter :: nlim = $nlim\t\t\t\t\t\t\t! number of limits over which the net flux is recorded\n";
	...
	print SYST "integer, parameter :: n_variables = $n_variable_parameters\n";
	if($variable_temp){
		print SYST "logical, parameter :: variable_temp = .true.\n";
	}else{
		print SYST "logical, parameter :: variable_temp = .false.\n";
        if(!$no_ambient_params_in_syst){
            if($temperature =~ m/\./){
                $str_temp = "";
            }else{
                $str_temp = ".";
            }
            print SYST "real(kind(1.d0)), parameter :: temp = $temperature" . $str_temp . "d0\t\t\t\t! temperature in K\n";
        }
	}
    if($variable_cs){
        print SYST "logical, parameter :: variable_cs = .true.\n";
    }else{
        print SYST "logical, parameter :: variable_cs = .false.\n";
    }
    if(!$disable_coag_sinks){
    ...
    if($variable_ion_source){
        print SYST "logical, parameter :: variable_ipr = .true.\n";
    }else{
        print SYST "logical, parameter :: variable_ipr = .false.\n";
    }
    if(!$no_ambient_params_in_syst){
        if($rh =~ m/\./){
            $str_temp = "";
        ...
        print SYST "real(kind(1.d0)), parameter :: rh = $rh" . $str_temp . "d0\t\t\t\t\t! RH in %\n";
        if($pw =~ m/\./){
            $str_temp = "";
        }else{
            $str_temp = ".";
        }
        print SYST "real(kind(1.d0)), parameter :: pw = $pw" . $str_temp . "d0\t\t\t\t\t! partial pressure of water in Pa\n";
    }
	print SYST "\n";
	
	print SYST "integer, parameter :: n_monomer_types = $nmolecules\n";
	print SYST "integer, parameter :: n_charges = $ncharges\t\t\t! number of charging states\n";
	...
	if(!$lloop){
		...
		if($l_save_outgoing){
			...
			$str_temp = "(/$n_flux_out, ";
			if($include_negative_ion_terms){
				$str_temp .= "$n_flux_out_neg, ";
			}
			if($include_positive_ion_terms){
				$str_temp .= "$n_flux_out_pos, ";
			}
			$str_temp =~ s/, $//;
			$str_temp .= "/)";
			$str_temp =  "integer, parameter :: nout_all($ncharges) = " . $str_temp;
			$str_temp .=  ", ";
			for($k=$n_flux_out; $k<=$n_flux_out_max; $k++){
				$str_temp .= "n$label[$k] = $k, ";
			}
			$str_temp =~ s/, $//;
			print SYST "$str_temp\t\t\t! indices for outgoing fluxes\n\n";
		}
        print SYST "integer, parameter :: nclust_coag_saved = $n_coag_per_clust\t\t\t! number of saved scavenged clusters\n";
        print SYST "integer, parameter :: nclust_out_saved = $n_out_clust\t\t\t\t! number of saved outgrown clusters\n";
        print SYST "integer, parameter :: nrange_coag_saved(2) = (/$range_coag_per_clust[0], $range_coag_per_clust[1]/)	! index range of saved scavenged clusters\n";
        print SYST "integer, parameter :: nrange_out_saved(2) = (/$range_out_clust[0], $range_out_clust[1]/)	! index range of saved outgrown clusters\n";
	}else{
		if(!$lloop_restrict && !$lloop_cs){
			print SYST "integer, parameter :: nout_all($ncharges) = $n_flux_out\t\t\t! indices for outgoing fluxes\n";
		}else{
			print SYST "integer, parameter :: nout_all($ncharges) = -1\t\t\t! not pre-determined\n";
		}
	}
	print SYST "\n";
	...
	if($l_old_output){
		...
		print DRIVER "%function [C, T, converged, clust, Cf, labels_ch, clust_flux, J_out, flux, outflux_matrix, out, sources] = $driver_function_name(Tmax, {C0, {T0}}, {'File', Filename, {...}}))\n";
    ...
    print DRIVER "% The rest of optional input parameters are given either as a single keyword (e.g. 'no_fluxes' or 'repeat'), or pairs of keyword and value\n% (e.g. 'Sources_in', filename or 'Options', string); see the ACDC Technical manual for a list of available parameters\n";
	if($variable_temp){
	...
	if(!$l_old_output){
		print DRIVER "% converged:	  1 if the simulation has converged, and 0 if not, and -1 if there are negative concentrations lower than a threshold value\n";
	}
	print DRIVER "% clust:		  cluster names\n";
	print DRIVER "% Cf:          final cluster distribution in cm^(-3)\n";
	print DRIVER "% labels_ch:	  names of the neutral and charged forms of all molecules, removed or extra protons and generic ions\n";
	print DRIVER "% clust_flux:	  names for all fluxes, including also non-clusters (sources, losses etc.)\n";
...
#	print DRIVER "%     		  DO NOT trust this 100% because it hasn't really been used or tested\n";
	if($l_old_output){
		print DRIVER "% converged:	  1 if the simulation has converged, 0 if not, and -1 if there are negative concentrations lower than a threshold value\n\n";
	}

}

if(!$lloop){
	# Printing info on the cluster set to the output files
	$lines{'nof1'} = "";	# Number of molecules of the first type in each cluster (including possible ionized form)
	$lines{'1of1'} = "";	# Indices of electrically neutral clusters containing one molecule of the first type
	$lines{'0'} = "";	# Indices of all electrically neutral species
	$lines{'n'} = "";	# Indices of all negative species
	$lines{'p'} = "";	# Indices of all positive species
	$lines{'0m'} = "";	# Indices of electrically neutral monomers
	$lines{'nm'} = "";	# Indices of negative monomers
	$lines{'pm'} = "";	# Indices of positive monomers
	$lines{'0c'} = "";	# Indices of electrically neutral clusters
	$lines{'nc'} = "";	# Indices of negative clusters
	$lines{'pc'} = "";	# Indices of positive clusters
	$lines{'cst'} = "";	# Charging states (1 = neutral, 2 = negative, 3 = positive)
	$lines{'m'} = "";	# Masses
	$lines{'d'} = "";	# Mass diameters
	$lines{'dm'} = "";	# Mobility diameters
	$lines{'cl'} = "";	# Cluster names
	$lines{'mon'} = "";	# Indices of monomers
	$lines{'nmon'} = "";	# Indices of non-monomer clusters
	$lines{'t0'} = "";	# Indices for the table output (neutral)
	$lines{'tn'} = "";	# Indices for the table output (negative)
	$lines{'tp'} = "";	# Indices for the table output (positive)
	$lines{'out0'} = "";	# Criteria for clusters growing out of the system (neutral)
	$lines{'outn'} = "";	# Criteria for clusters growing out of the system (negative)
	$lines{'outp'} = "";	# Criteria for clusters growing out of the system (positive)
	$i = 0;
	$j = 0;
	$k = 0;
	$l = 0;
	...
	for($iclus=1;$iclus<=$max_cluster_number;$iclus++){
		...
		$mass1 = sprintf("%.4f",$mass1/$mass_conv);
		$clust_mass_max = max($clust_mass_max,$mass1);
		$mass1 =~ s/e/d/;
		$lines{'m'} .= "$mass1, ";
		$radius = $clust_radius[$iclus][0];
		$diameter = sprintf("%.4f",$radius*2e9);
		$diameter_max = max($diameter_max,$diameter);
		$diameter =~ s/e/d/;
		$lines{'d'} .= "$diameter, ";
		$diameter = sprintf("%.4f",(2.0e9*$radius+0.3)*sqrt(1+28.8*$mass_conv/$mass1));
		...
		}elsif($iclus==$n_generic_neg){
			$lines{'cst'} .= "2, ";
			$lines{'n'} .= "$iclus, ";
			$k += 1;
			$lines{'nof1'} .= "0, ";
		}elsif($iclus==$n_generic_pos){
			$lines{'cst'} .= "3, ";
			$lines{'p'} .= "$iclus, ";
			$l += 1;
			$lines{'nof1'} .= "0, ";
	...
	if($lfortran){
		...
		print SYST "integer, parameter :: n_negatives = $k\t\t\t! negative\n";
		print SYST "integer, parameter :: n_positives = $l\t\t\t! positive\n";
        print SYST "integer, parameter :: nclust_nogen = $max_cluster\t\t\t! number of clusters and molecules excluding generic ions\n";
		print SYST "\ninteger, parameter :: n_neutral_monomers = $jm, neutral_monomers($jm) = (/$lines{'0m'}/)\t\t\t! number and indices of neutral monomers\n";
		print SYST "integer, parameter :: n_negative_monomers = $km";
		if($km>0){
			print SYST ", negative_monomers($km) = (/$lines{'nm'}/)";
		}
		print SYST "\t\t\t! negative\n";
		print SYST "integer, parameter :: n_positive_monomers = $lm";
		if($lm>0){
			print SYST ", positive_monomers($lm) = (/$lines{'pm'}/)";
		}
		print SYST "\t\t\t! positive\n";
		print SYST "\ninteger, parameter :: n_neutral_clusters = $jc\t\t\t! number of neutral clusters\n";
		print SYST "integer, parameter :: n_negative_clusters = $kc\t\t\t! negative\n";
		print SYST "integer, parameter :: n_positive_clusters = $lc\t\t\t! positive\n\n";
	
		$clust_mass_max = sprintf("%.4f",$clust_mass_max);
		$clust_mass_max =~ s/e/d/;
		print SYST "real(kind(1.d0)), parameter :: mass_max = $clust_mass_max\n";
		$diameter_max = sprintf("%.4f",$diameter_max);
		$diameter_max =~ s/e/d/;
		print SYST "real(kind(1.d0)), parameter :: diameter_max = $diameter_max\n";
		$mob_diameter_max = sprintf("%.4f",$mob_diameter_max);
		$mob_diameter_max =~ s/e/d/;
		print SYST "real(kind(1.d0)), parameter :: mob_diameter_max = $mob_diameter_max\n";
		
		# Maximum molecular content by charge (currently not needed)
		# print SYST "integer, parameter :: nmols_max($ncharges, $nmol_types_used) = reshape((/" . join(',',@n_max_type[@mol_types_used[1..$nmol_types_used]]);
		# if($include_negative_ion_terms){
			# print SYST "," . join(',',@n_max_type_neg[@mol_types_used[1..$nmol_types_used]]);
		# }
		# if($include_positive_ion_terms){
			# print SYST "," . join(',',@n_max_type_pos[@mol_types_used[1..$nmol_types_used]]);
		# }
		# print SYST "/),(/$ncharges, $nmol_types_used/))\t\t! maximum molecular content by charge\n";
        
		# Overall maximum molecular content
		print SYST "integer, parameter :: ij_ind_max($nmol_types_used) = (/";
		$str_temp = "";
		for($imol=1;$imol<=$nmol_types_used;$imol++){
			$str_temp .= max($n_max_type[$mol_types_used[$imol]],$n_max_type_neg[$mol_types_used[$imol]],$n_max_type_pos[$mol_types_used[$imol]]) . ", ";
		}
		$str_temp =~ s/, $//;
		print SYST "$str_temp/)\t\t! maximum molecular content\n";
        
        # Clusters that are at the boundary of the system (i.e. at least one component has its maximum molecular number)
        $n_at_bound = 0;
        @clus_at_bound = ();
        for($iclus=1;$iclus<=$max_cluster;$iclus++){
            @temp_array = @{ $clust_comp[$iclus] };
            $lfound = 0;
            for($imol=1;$imol<=$nmol_types_used;$imol++){
                if ($charge[$mol_types_used[$imol]] != 0){
                    next;
                }
                if($lneutral_cluster[$iclus]){
                    if($n_max_type[$mol_types_used[$imol]]==0){
                        next;
                    }elsif($temp_array[$mol_types_used[$imol]]==$n_max_type[$mol_types_used[$imol]]){
                        $lfound = 1;
                        last;
                    }
                }elsif($lnegative_cluster[$iclus]){
                    if($n_max_type_neg[$mol_types_used[$imol]]==0){
                        next;
                    }elsif($temp_array[$mol_types_used[$imol]]==$n_max_type_neg[$mol_types_used[$imol]]){
                        $lfound = 1;
                        last;
                    }
                }elsif($lpositive_cluster[$iclus]){
                    if($n_max_type_pos[$mol_types_used[$imol]]==0){
                        next;
                    }elsif($temp_array[$mol_types_used[$imol]]==$n_max_type_pos[$mol_types_used[$imol]]){
                        $lfound = 1;
                        last;
                    }
                }
            ...
            if($lfound){
                $n_at_bound += 1;
                push(@clus_at_bound,$iclus);
            }
        }
        print SYST "integer, parameter :: n_bound = $n_at_bound\t\t! number of clusters at the system boundary\n";
        print SYST "integer, parameter :: n_comp_out = $n_comp_out\t\t! number of clusters growing out in different compositions\n";	

		if($lines{'out0'} !~ /^\s*$/){
			print SYST "\ninteger, parameter :: nmols_out_neutral($n_nucl_rules, $nmol_types_used) = $lines{'out0'}\t\t\t! criteria for outgrowing neutrals\n";
		}
		if($lines{'outn'} !~ /^\s*$/){
			print SYST "integer, parameter :: nmols_out_negative($n_nucl_rules_neg, $nmol_types_used) = $lines{'outn'}\t\t\t! negatives\n";
		}
		if($lines{'outp'} !~ /^\s*$/){
			print SYST "integer, parameter :: nmols_out_positive($n_nucl_rules_pos, $nmol_types_used) = $lines{'outp'}\t\t\t! positives\n";
		}
		...
		$lines{'c_size'} = '';
		$lines{'c_def'} = '';
		print SYST "\n\ncontains\n\nsubroutine n_$molecule_name[0]_in_clusters$append_to_syst_names(n_$molecule_name[0])\n\timplicit none\n";
		print SYST "\tinteger :: n_$molecule_name[0]($max_cluster_number)\n\n";
		print SYST "\tn_$molecule_name[0] = (/$lines{'nof1'}/)\n\n";
		print SYST "end subroutine n_$molecule_name[0]_in_clusters$append_to_syst_names\n\n";
		print SYST "subroutine clusters_with_1_$molecule_name[0]$append_to_syst_names(cluster_numbers)\n\timplicit none\n";
		print SYST "\tinteger :: cluster_numbers($i)\n\n\tcluster_numbers = (/$lines{'1of1'}/)\n\n";
		print SYST "end subroutine clusters_with_1_$molecule_name[0]$append_to_syst_names\n\n";
		...
		$lines{'a_inp'} =~ s/, $//;
		if($lines{'a_inp'} ne ''){
			print SYST "subroutine arrays$append_to_syst_names($lines{'a_inp'})\n\timplicit none\n";
			$lines{'a_size'} =~ s/, $//;
			print SYST "\tinteger :: $lines{'a_size'}\n\n";
			print SYST $lines{'a_def'};
			print SYST "\nend subroutine arrays$append_to_syst_names\n\n";
		}
		$lines{'c_inp'} =~ s/, $//;
		print SYST "subroutine cluster_arrays$append_to_syst_names($lines{'c_inp'})\n\timplicit none\n";
		$lines{'c_size'} =~ s/, $//;
		print SYST "\tinteger :: $lines{'c_size'}\n\n";
		print SYST $lines{'c_def'};
		print SYST "\nend subroutine cluster_arrays$append_to_syst_names\n\n";
		

		print SYST "subroutine get_charging_state$append_to_syst_names(charging_state)\n\timplicit none\n";
		print SYST "\tinteger :: charging_state($max_cluster_number)\n\n\tcharging_state = (/$lines{'cst'}/)\n";
		print SYST "\nend subroutine get_charging_state$append_to_syst_names\n\n";

	
		print SYST "subroutine get_mass$append_to_syst_names(mass)\n\timplicit none\n";
		print SYST "\treal(kind(1.d0)) :: mass($max_cluster_number)\n\n\tmass = (/$lines{'m'}/)\n";
		print SYST "\nend subroutine get_mass$append_to_syst_names\n\n";
	
	
		print SYST "subroutine get_diameter$append_to_syst_names(diameter)\n\timplicit none\n\n";
		print SYST "\treal(kind(1.d0)) :: diameter($max_cluster_number)\n\n\t diameter = (/$lines{'d'}/)\t! dry value\n";
		print SYST "\nend subroutine get_diameter$append_to_syst_names\n\n";
	
	
		print SYST "subroutine get_mob_diameter$append_to_syst_names(mob_diameter)\n\timplicit none\n\n";
		print SYST "\treal(kind(1.d0)) :: mob_diameter($max_cluster_number)\n\n\t mob_diameter = (/$lines{'dm'}/)\t! dry value\n";
		print SYST "\nend subroutine get_mob_diameter$append_to_syst_names\n\n";
	
		print SYST "subroutine cluster_names$append_to_syst_names(clust)\n\timplicit none\n\tcharacter(len=11), dimension(".min($size_C_out,$max_n_flux).") :: clust\n";
        print SYST "\n";
        ...
        if($l_save_coag_per_clust || $l_save_outgoing_clust){
            print SYST "\n\t! Additional flux elements not listed here:\n";
            if($l_save_coag_per_clust){
                print SYST "\t! Elements from $range_coag_per_clust[0] to $range_coag_per_clust[1]: coagulation loss of each cluster\n";
            }
            if($l_save_outgoing_clust){
                print SYST "\t! Elements from $range_out_clust[0] to $range_out_clust[1]: each outgrown cluster composition\n";
            }
        }
		print SYST "\nend subroutine cluster_names$append_to_syst_names\n\n";
		...
		$line = "\t% molecule names\n\tlabels = {";
		$line2 = "\t% neutral, negative and positive species\n\tlabels_ch = {";
		for($imol=1;$imol<=$nmol_types_used;$imol++){
			$line = $line . "'" . $mol_names_used[$imol] . "' ";
			$jmol = $mol_types_used[$imol];
			if($charge[$jmol] == 0){
				$line2 = $line2 . "'" . $molecule_name[$jmol] . "' ";
				if($n_corr_negative_mol[$jmol] ~~ @mol_types_used){
					$line2 = $line2 . "'" . $corr_negative_mol[$jmol] . "' ";
				}elsif($missing_proton ~~ @mol_types_used && &compare_clusters($corr_negative_mol[$jmol],"1$molecule_name[$jmol]1$label_missing_proton")){
					$line2 = $line2 . "'" . $label_missing_proton . "' ";
				}else{
					$line2 = $line2 . "'' ";
				}
				if($n_corr_positive_mol[$jmol] ~~ @mol_types_used){
					$line2 = $line2 . "'" . $corr_positive_mol[$jmol] . "' ";
				}elsif($proton ~~ @mol_types_used && &compare_clusters($corr_positive_mol[$jmol],"1$molecule_name[$jmol]1$label_proton")){
					$line2 = $line2 . "'" . $label_proton . "' ";
				}else{
					$line2 = $line2 . "'' ";
				}
				$line2 = $line2 . "; ";
			}elsif($jmol != $proton && $jmol != $missing_proton && !($n_corr_neutral_mol[$jmol] ~~ @mol_types_used)){
				if($charge[$jmol] == -1){
					$line2 = $line2 . "'' '" . $molecule_name[$jmol] . "' ''";
				}elsif($charge[$jmol] == 1){
					$line2 = $line2 . "'' '' '" . $molecule_name[$jmol] . "'";
		...
		$line = "$line};\n";
		print DRIVER $line;
		# "Cluster-phase ions", i.e. missing or extra protons
		$line2 = $line2 . "'' ";
		if($missing_proton ~~ @mol_types_used){
			$line2 = $line2 . "'" . $label_missing_proton . "' ";
		}else{
			$line2 = $line2 . "'' ";
		}
		if($proton ~~ @mol_types_used){
			$line2 = $line2 . "'" . $label_proton . "' ";
		}else{
			$line2 = $line2 . "'' ";
		}
		$line2 = $line2 . "; ";
		# "Gas-phase ions", i.e. generic charger ions
		$line2 = $line2 . "'' ";
		if($include_generic_neg){
			$line2 = $line2 . "'" . $label_generic_neg . "' ";
		}else{
			$line2 = $line2 . "'' ";
		}
		if($include_generic_pos){
			$line2 = $line2 . "'" . $label_generic_pos . "' ";
		}else{
			$line2 = $line2 . "'' ";
		}
		$line2 =~ s/ $//;
		...
		print DRIVER "\tcharging_states = [$lines{'cst'}]; % charging states (1 = neutral, 2 = negative, 3 = positive)\n";
		...
		print DRIVER "\tneutral_monomers = [$lines{'0m'}];\n";
		print DRIVER "\tnegative_monomers = [$lines{'nm'}];\n";
		print DRIVER "\tpositive_monomers = [$lines{'pm'}];\n";
	...
	print SYST "integer, parameter :: ij_ind_max($nmol_types_used) = (/$nmol_ranges_list/)\n";
	if($nmol_types_used>1 && $lloop_cs){
		print SYST "integer, parameter :: ij_ind_cs($nmol_types_used) = (/$nmol_cs_list/)\n";
	}
...
if($lfortran){
	print SYST "subroutine monomer_names$append_to_syst_names(clust_mon)\n\timplicit none\n";
	print SYST "\tcharacter(len=11), dimension($nmolecules) :: clust_mon\n\n";
	for($imol=1;$imol<=$nmolecules;$imol++){
		if(!$lloop){
			$str_temp = $label[$molecules[$imol]];
		}else{
			$str_temp = "1".$mol_names_used[$imol]; # loop mode: no charged components, for now
		}
		print SYST "\tclust_mon($imol)(:) = '$str_temp'\n";
	}
	print SYST "\nend subroutine monomer_names$append_to_syst_names\n\n";

	print SYST "subroutine molecule_names$append_to_syst_names(labels)\n\timplicit none\n";
	print SYST "\tcharacter(len=11), dimension($nmol_types_used) :: labels\n\n";
	for($imol=1;$imol<=$nmol_types_used;$imol++){
		print SYST "\tlabels($imol)(:) = '$mol_names_used[$imol]'\n";
	}
	print SYST "\nend subroutine molecule_names$append_to_syst_names\n\n";
	
	print SYST "subroutine monomer_indices$append_to_syst_names(n_monomers)\n\timplicit none\n";
	print SYST "\tinteger :: n_monomers($nmolecules)\n";
	if($lloop && $nmol_types_used>1){
		print SYST "\tinteger :: clust_from_indices($nmol_comp_list), i, ij_ind($nmol_types_used)\n\n";
        # This may be inconsistent if the equations and system files have different subroutine suffices - but
        # such setting doesn't sound very likely for the loop mode, so just print out a warning and deal with this if need be
        if($append_to_eq_names ne $append_to_syst_names){
            print "Warning: calling a subroutine with suffix \"$append_to_eq_names\" in file $system_file_name_fortran which contains subroutines with suffix \"$append_to_syst_names\".\n";
        }
		print SYST "\tcall get_cluster_numbers$append_to_eq_names(clust_from_indices)\n";
		print SYST "\tn_monomers = 0\n\n";
		print SYST "\tdo i = 1,$nmol_types_used\n";
		print SYST "\t\tij_ind = 0\n";
		print SYST "\t\tij_ind(i) = 1\n";
		print SYST "\t\tn_monomers(i) = clust_from_indices($nmol_ranges_ij_ind)\n";
		print SYST "\tend do\n\n";
	}else{
		print SYST "\n\tn_monomers = (/$monomer_indices/)\n\n";
	}
	print SYST "end subroutine monomer_indices$append_to_syst_names\n\n";
    
    if(!$lloop){
        # Compositions and indices of clusters at the system boundary
        print SYST "subroutine get_bound$append_to_syst_names(bound_clusters,nmols_bound)\n\timplicit none\n";
        print SYST "\tinteger :: bound_clusters($n_at_bound), nmols_bound($n_at_bound,$nmol_types_used)\n\n";
        $str_temp = "\tbound_clusters = (/";
        for($i=0;$i<$n_at_bound;$i++){
            $iclus = $clus_at_bound[$i];
            print SYST "\tnmols_bound(" . ($i+1) . ",:) = (/" . join(', ',@{ $clust_comp[$iclus] }[@mol_types_used[1..$nmol_types_used]]) . "/)\n";
            $str_temp .= "$iclus, ";
		}
        $str_temp =~ s/, $//;
        print SYST "\n" . $str_temp . "/)\n\n";
        print SYST "end subroutine get_bound$append_to_syst_names\n\n";
        
        # Diameters and compositions of individual clusters growing out of the system
        print SYST "subroutine get_diameter_out$append_to_syst_names(diameter_out)\n\timplicit none\n";
        print SYST "\treal(kind(1.d0)) :: diameter_out($n_comp_out)\n\n";
        $lines{'d'} = "";	# Mass diameters of the ougrowing clusters
        $jclus = 0;
        for($iclus=1;$iclus<=$n_comp_out;$iclus++){
            if($jclus==10){
                $lines{'d'} .= "&\n\t\t&";
                $jclus = 0;
            }
            $jclus += 1;
            $diameter = sprintf("%.4f",&calculate_radius(\@{ $clust_comp_out[$iclus] })*2e9);
            $diameter =~ s/e/d/;
            $lines{'d'} .= "$diameter, ";
        }
        $lines{'d'} =~ s/, $//;
        print SYST "\tdiameter_out = (/$lines{'d'}/)\t! dry value\n\n";
        print SYST "end subroutine get_diameter_out$append_to_syst_names\n\n";
        
        print SYST "subroutine get_nmols_out$append_to_syst_names(nmols_out)\n\timplicit none\n";
        print SYST "\tinteger :: nmols_out($n_comp_out,$nmol_types_used)\n\n";
        for($iclus=1;$iclus<=$n_comp_out;$iclus++){
            print SYST "\tnmols_out($iclus,:) = (/" . join(', ',@{ $clust_comp_out[$iclus] }[@mol_types_used[1..$nmol_types_used]]) . "/)\n";
        }
        print SYST "\n";
        print SYST "end subroutine get_nmols_out$append_to_syst_names\n\n";
        
        # Cluster composition as neutral molecules, i.e. charged molecules are counted as their neutral counterparts
        print SYST "subroutine molecule_names_nocharge$append_to_syst_names(labels_nocharge)\n\timplicit none\n";
        print SYST "\tcharacter(len=11), dimension($jm) :: labels_nocharge\n\n";
        $i = 0;
        for($imol=1;$imol<=$nmol_types_used;$imol++){
            if($charge[$mol_types_used[$imol]] == 0){
                $i += 1;
                print SYST "\tlabels_nocharge($i)(:) = '$molecule_name[$mol_types_used[$imol]]'\n";
            }
        }
        print SYST "\nend subroutine molecule_names_nocharge$append_to_syst_names\n\n";
        # Get the neutral-equivalent composition for both the simulated and the outgrown clusters
        for($ii=0;$ii<=1;$ii++){
            if($ii == 0){
                # Simulated clusters
                $str_temp = "";
                $str_temp2 = "_clust";
                $temp_val = $max_cluster_number;
                $temp_val2 = $max_cluster;
            }else{
                # Outgrown clusters
                $str_temp = "_out";
                $str_temp2 = "_out";
                $temp_val = $n_comp_out;
                $temp_val2 = $n_comp_out;
            }
            print SYST "subroutine get_nmols_nocharge$str_temp$append_to_syst_names(nmols_nocharge$str_temp2)\n\timplicit none\n";
            print SYST "\tinteger :: nmols_nocharge$str_temp2($temp_val,$jm)\n\n";
            print SYST "\tnmols_nocharge$str_temp2 = 0\n\n";
            for($iclus=1;$iclus <= $temp_val2;$iclus++){
                @temp_array = (0) x $jm;
                if($ii == 0){
                    @temp_array_2 = @{ $clust_comp[$iclus] };
                }else{
                    @temp_array_2 = @{ $clust_comp_out[$iclus] };
                }
                $i = -1;
                for($imol=1;$imol<=$nmol_types_used;$imol++){
                    if($charge[$mol_types_used[$imol]] == 0){
                        $i += 1;
                        $temp_array[$i] += $temp_array_2[$mol_types_used[$imol]];
                        if($n_corr_negative_mol[$mol_types_used[$imol]] >= 0){
                            $temp_array[$i] += $temp_array_2[$n_corr_negative_mol[$mol_types_used[$imol]]];
                        }
                        if($n_corr_positive_mol[$mol_types_used[$imol]] >= 0){
                            $temp_array[$i] += $temp_array_2[$n_corr_positive_mol[$mol_types_used[$imol]]];
                        }
                    }
                }
                print SYST "\tnmols_nocharge$str_temp2($iclus,:) = (/" . join(', ',@temp_array) . "/)\n";
            }
            print SYST "\n";
            print SYST "end subroutine get_nmols_nocharge$str_temp$append_to_syst_names\n\n";
        }
    }
    
	print SYST "\nend module acdc_system$append_to_syst_names\n\n";
	close(SYST);
}
...
if($lfortran){
	...
	if($lloop){
		print OUT "!!!!!!!!!!!!!!!!!!!!!!!! KEY !!!!!!!!!!!!!!!!!!!!!!!!\n!\n";
		if($nmol_types_used==1){
			print OUT "! $max_cluster clusters with 1-$n_max_type[$mol_types_used[1]] $molecule_name[$mol_types_used[1]] molecules\n";
		}else{
			if($lloop_restrict){
				print OUT "! Cluster set will be restricted according to the criteria given by the user\n";
				print OUT "! Available clusters from which the set is selected:\n";
			}
			...
			if($l_save_outgoing){
				print OUT "! and the last element is the outgoing flux\n";
			}
		}
		if($l_j_in){
        ...
        if($l_save_coag_per_clust){
            print OUT "! $range_coag_per_clust[0]-$range_coag_per_clust[1] are for coagulation losses of each cluster\n";
        }
        if($l_save_outgoing_clust){
            print OUT "! $range_out_clust[0]-$range_out_clust[1] are for collisions that lead out of the system to different cluster compositions\n";
        }
		print OUT "!!!!!!!!!!!!!!!!!!!!!! END KEY !!!!!!!!!!!!!!!!!!!!!!\n\n";
	...
	if($lloop && !$lloop_cs && !$l_save_outgoing){
		print OUT "subroutine feval$append_to_eq_names(neqn,t,c,f_out,coef,ipar)\n";
	}else{
		print OUT "subroutine feval$append_to_eq_names(neqn,t,c,f,coef,ipar)\n";
	}
	if($lloop){
		if($nmol_types_used>1){
			print OUT "use acdc_system$append_to_syst_names, only : monomer_indices$append_to_syst_names\n";
		}
		print OUT "use acdc_simulation_setup$append_to_ext_names, only : sources_and_constants$append_to_ext_names\n";
		if($l_j_in){
	...
	if($lloop_cs){
		if($nmol_types_used==1){
			print OUT ", n_cs = $max_cluster_number";
		}else{
			print OUT ", ij_ind_cs($nmol_types_used) = (/$nmol_cs_list/)";
		}
	}
	print OUT "\n";
	...
	if($lloop && !$lloop_cs && !$l_save_outgoing){
		print OUT "\treal(kind(1.d0)) :: t, c(neqn), f_out(neqn), f(neqn+1)";
	}else{
		print OUT "\treal(kind(1.d0)) :: t, c(neqn), f(neqn)";
	}
	if($l_mcmc_any){
		print OUT ", coef(" . ($n_mcmc_coefs+$variable_cs+2*$variable_ion_source) . ")";
	}elsif($l_variable_parameters){
		print OUT ", coef($n_variable_parameters)";
	}elsif($lloop && $nmol_types_used>1){
		print OUT ", coef($nmol_types_used)";
	}elsif($lloop){
		print OUT ", coef";
	...
	if($lloop){
		if($lloop_restrict){
			print OUT "\tlogical, save, allocatable :: isconst(:)\n";
			print OUT "\treal(kind(1.d0)), save, allocatable :: K(:,:)";
			if(!$disable_evap){
				print OUT ", E(:,:)";
			}
			if($l_use_get_losses){
			...
			print OUT ", source(:)\n";
			print OUT "\tinteger, save, allocatable :: fitted(:,:)\n";
		}else{
			print OUT "\tlogical, save :: isconst($max_cluster_number) = .false.\n";
			...
			print OUT ", source($max_cluster_number)\n";
			print OUT "\tinteger, save :: fitted($max_cluster_number,0:$max_cluster_number)=0\n";
		}
		if($nmol_types_used>1){
			print OUT "\tinteger, save :: nclust_inc=0, indices(nclust,$nmol_types_used)";
			print OUT ", clust_from_indices($nmol_comp_list)";
			print OUT ", n_monomers($nmol_types_used)\n";
		}
	}else{
		print OUT "\tlogical, save :: isconst($size_C_eqn) = .false.\n";
		print OUT "\treal(kind(1.d0)), save :: coef_quad(nclust,nclust,$size_ind_coef)=0.d0,coef_lin($size_ind_coef,$size_ind_coef,nclust)=0.d0,source($size_C_eqn)=0.d0\n";
		print OUT "\tinteger, save :: ind_quad_loss($size_ind_coef,0:$ind_quad_loss[0])=0,ind_quad_form($size_ind_coef,0:$ind_quad_form[0])=0\n";
		print OUT "\tinteger, save :: ind_quad_loss_extra($size_ind_coef,0:$ind_quad_loss_extra[0])=0,ind_quad_form_extra($size_ind_coef,0:$ind_quad_form_extra[0])=0\n";
		print OUT "\tinteger, save :: ind_lin_loss($size_ind_coef,0:$ind_lin_loss[0])=0,ind_lin_form($size_ind_coef,0:$ind_lin_form[0])=0,fitted($size_C_eqn,0:$size_C_eqn)=0\n";
        if($l_save_outgoing_clust){
            print OUT "\tinteger, save :: ind_out_clust(nclust,nclust)=0\n";
        }
		if($l_reac_in_clus){
			print OUT "\tinteger, save :: ind_reac_loss($size_ind_coef,0:$ind_reac_loss[0])=0,ind_reac_form($size_ind_coef,0:$ind_reac_form[0])=0\n";
		}
		print OUT "\treal(kind(1.d0)), save :: n_quad_form_extra($size_ind_coef,0:" . ($ind_quad_form_extra[0])/3 . ")=0.d0\n";
	}
	print OUT $lines{'source_def'};
	print OUT "\treal(kind(1.d0)), parameter :: pi=4.d0*atan(1.d0)\n";
	if($lloop && ($nmol_types_used>1 || $l_j_in)){
		print OUT "\n\t! system size parameters\n";
		if($nmol_types_used>1){
			print OUT "\tif (nclust_inc .eq. 0) then\n";
			print OUT "\t\tcall get_molecule_numbers$append_to_eq_names(nclust_inc,indices)\n";
			print OUT "\t\tcall get_cluster_numbers$append_to_eq_names(clust_from_indices)\n";
			print OUT "\t\tcall monomer_indices$append_to_syst_names(n_monomers)\n";
			if($lloop_restrict){
				print OUT "\t\tallocate(isconst(nclust_inc))\n";
				print OUT "\t\tallocate(fitted(nclust_inc,0:nclust_inc))\n";
				print OUT "\t\tallocate(K(nclust_inc,nclust_inc))\n";
				if(!$disable_evap){
					print OUT "\t\tallocate(E(nclust_inc,nclust_inc))\n";
				}
				if($l_use_get_losses){
					print OUT "\t\tallocate(loss(nclust_inc))\n";
				}
				print OUT "\t\tallocate(source(nclust_inc))\n";
			}
			print OUT "\tend if\n";
		}
		if($l_j_in){
			print OUT "\tif (n_j_in .eq. 0) then\n";
			print OUT "\t\tdo i = 1,nclust\n";
			print OUT "\t\t\tcall get_masses_and_radii$append_to_eq_names(m,r,i)\n";
	...
	if($lloop){
		print OUT "\t\tcall sources_and_constants$append_to_ext_names($nclust_included,source,isconst,fitted)\n";
		if($l_mcmc_any){
			print OUT "\t\tcall get_coll$append_to_eq_names($nclust_included,K,coef)\n";
		}elsif( $variable_temp){
			print OUT "\t\tcall get_coll$append_to_eq_names($nclust_included,K,coef(1))\n";
		}else{
			print OUT "\t\tcall get_coll$append_to_eq_names($nclust_included,K)\n";
		}
		if(!$disable_evap){
			if($l_mcmc_any){
				print OUT "\t\tcall get_evap$append_to_eq_names($nclust_included,E,coef)\n";
			}elsif($variable_temp){
				print OUT "\t\tcall get_evap$append_to_eq_names($nclust_included,E,K,coef(1))\n";
			}else{
				print OUT "\t\tcall get_evap$append_to_eq_names($nclust_included,E)\n";
			}
		}
		# Add the factor that deals with the permutations for processes involving two similar clusters
		# (For speed, it's added here instead of inside the loop)
		if($disable_nonmonomers && $nmol_types_used>1){
			# If only monomer processes are included, add the factor to all monomer-monomer processes,
			# as processes between different types of monomers will be otherwise counted twice in the loop
			print OUT "\t\tdo i = 1,$nmol_types_used\n";
			print OUT "\t\t\tdo j = 1,$nmol_types_used\n";
			print OUT "\t\t\t\tK(n_monomers(i),n_monomers(j)) = 0.5d0*K(n_monomers(i),n_monomers(j))\n";
			print OUT "\t\t\t\tK(n_monomers(j),n_monomers(i)) = K(n_monomers(i),n_monomers(j))\n";
			print OUT "\t\t\tend do\n";
			print OUT "\t\tend do\n";
		}else{
			print OUT "\t\tdo i = 1,$nclust_included\n";
			print OUT "\t\t\tK(i,i) = 0.5d0*K(i,i)\n";
			print OUT "\t\tend do\n";
		}
		if($l_use_get_losses){
			print OUT "\t\tcall get_losses$append_to_eq_names($nclust_included,loss";
			if($variable_cs){
				print OUT ",coef";
			}
			print OUT ")\n";
		...
		print OUT "\t\tcall initialize_parameters$append_to_eq_names(coef_quad,coef_lin,ind_quad_loss,ind_quad_form,&\n\t\t&\tind_lin_loss,ind_lin_form,ind_quad_loss_extra,ind_quad_form_extra,&\n";
        if($l_save_outgoing_clust){
            print OUT "\t\t&\tind_out_clust,&\n";
        }
        print OUT "\t\t&\tsource,isconst,fitted";
		if($l_mcmc_any || $l_variable_parameters){
			print OUT ",coef";
		}
		print OUT ",ipar)\n";
	...
	print OUT "\tend if\n\n";
	if($lloop && !$l_variable_parameters){
		print OUT "\t! override the monomer sources given by the subroutine call\n";
		print OUT "\tsource($monomer_indices) = coef\n\n";
	}
	print OUT $lines{'source'};
    print OUT "\t! override the isconst settings, if needed\n";
	...
	if($lloop){
		...
		if($disable_nonmonomers){
			print OUT "\tdo i = 1,$nclust_included\n";
			print OUT "\t\tf(i) = f(i)+source(i)";
			if($l_use_get_losses){
				print OUT "-loss(i)*c(i)";
			}
			print OUT "\n\tend do\n";
			if($nmol_types_used>1){
				print OUT "\tdo imol = 1,$nmol_types_used\n";
				print OUT "\t\ti = n_monomers(imol)\n";
			}else{
				print OUT "\tdo i = 1,1\n";
			}
			if(!$l_j_in){
				print OUT "\t\tdo j = 1,$nclust_included\n";
			}else{
				print OUT "\t\tdo j = n_j_in,$nclust_included\n";
			}
		}else{
			if(!$l_j_in){
				print OUT "\tdo i = 1,$nclust_included\n";
			}else{
				print OUT "\tdo i = n_j_in,$nclust_included\n";
			...
			print OUT "\n";
			print OUT "\t\tdo j = i,$nclust_included\n";
		}
		if($nmol_types_used>1){
			print OUT "\t\t\tij_ind = indices(i,:)+indices(j,:)\n";
			print OUT "\t\t\tif (clust_from_indices($nmol_ranges_ij_ind) .eq. 0) then\n";
			if($lloop_cs){
				print OUT "\t\t\t\tij_ind = ij_ind_cs\n";
			}else{
				print OUT "\t\t\t\tij = $n_flux_out\n";
			}
			if ($lloop_boundary){
				print OUT "\t\t\telse if (clust_from_indices($nmol_ranges_ij_ind) .eq. -1) then\n";
				print OUT "\t\t\t\tcycle\n";
			}
			if($lloop_cs){
				print OUT "\t\t\tend if\n";
				print OUT "\t\t\tij = clust_from_indices($nmol_ranges_ij_ind)\n";
			}else{
				print OUT "\t\t\telse\n";
				print OUT "\t\t\t\tij = clust_from_indices($nmol_ranges_ij_ind)\n";
				print OUT "\t\t\tend if\n";
		...
		# Printing out the indices in the rate constant matrices as (j,i) instead of (i,j) for speed,
		# as Fortran is column-major
		print OUT "\t\t\tf(i) = f(i)-K(j,i)*c(i)*c(j)\n";
		print OUT "\t\t\tf(j) = f(j)-K(j,i)*c(i)*c(j)\n";
		print OUT "\t\t\tf(ij) = f(ij)+K(j,i)*c(i)*c(j)\n";
		# Create a string containing the condition for evaporation of cluster i+j (if (...) then)
        if($disable_nonmonomer_evaps && $nmol_types_used==1){
            $lines{'l_evap'} = "if (i .eq. 1) then\n";
        }else{
            $lines{'l_evap'} = "if (E(j,i) .gt. 0.d0) then\n";
        }
		if(!$disable_evap){
			print OUT "\t\t\t$lines{'l_evap'}";
			print OUT "\t\t\t\tf(i) = f(i)+E(j,i)*c(ij)\n";
			print OUT "\t\t\t\tf(j) = f(j)+E(j,i)*c(ij)\n";
			print OUT "\t\t\t\tf(ij) = f(ij)-E(j,i)*c(ij)\n";
			print OUT "\t\t\tend if\n";
		}
		print OUT "\t\tend do\n";
		...
		if($lgrowth_for_j_tests){
			...
			print OUT "\t\t\tj = 2\n";
			print OUT "\t\t\tij = min($n_flux_out,i+j)\n";
			print OUT "\t\t\tf(i) = f(i)-K(j,i)*c(i)*$c_growth_compound_str\n";
			print OUT "\t\t\tf(ij) = f(ij)+K(j,i)*c(i)*$c_growth_compound_str\n";
			print OUT "\n";
			print OUT "\t\t\tj = 3\n";
			print OUT "\t\t\tij = min($n_flux_out,i+j)\n";
			print OUT "\t\t\tf(i) = f(i)-K(j,i)*c(i)*$c_growth_compound_str\n";
		...
		if(!$lloop_cs && !$l_save_outgoing){
			print OUT "\tf_out = f(1:$nclust_included)\n";
		}
	}else{
		print OUT "\tf = 0.d0\n";
        $str_temp = "\tdo i = 1, neqn";
        if($l_save_coag_per_clust || $l_save_outgoing_clust){
            # Do not include the optional last elements that don't have their own coefficient / index arrays
            $str_temp .= "-".($n_coag_per_clust+$n_out_clust);
        }
        $str_temp .= " ! loop over all the clusters";
		if($include_generic_neg && $include_generic_pos){
            ...
            $str_temp .= " + generic charger ions";
		}
        if($l_save_outgoing){
            $str_temp .= " + other fluxes";
        }
        print OUT "$str_temp\n";
		...
		print OUT "\t\t\tend do\n";
		print OUT "\t\t\t! finally, add possible external sources\n";
		print OUT "\t\t\tf(i) = f(i) + source(i)\n";
		print OUT "\t\tend if\n";
		print OUT "\tend do\n";
        if($l_save_coag_per_clust){
            print OUT "\t! coagulation for each cluster\n";
            print OUT "\ti = $n_flux_coag\n";
            print OUT "\tdo n = 1, 2*ind_lin_form(i,0)-1, 2 ! loop over all coagulations\n";
            print OUT "\t\tj = ".($range_coag_per_clust[0]-1)."+ind_lin_form(i,n+1)\n";
            print OUT "\t\tf(j) = f(j)+coef_lin(i,ind_lin_form(i,n),ind_lin_form(i,n+1))*c(ind_lin_form(i,n+1))\n";
            print OUT "\tend do\n";
        }
        if($l_save_outgoing_clust){
            print OUT "\t! flux out for each outgrown cluster\n";
            print OUT "\tdo i = $n_flux_out, $n_flux_out_max ! loop over all charging states\n";
            print OUT "\t\tdo n = 1, 2*ind_quad_form(i,0)-1, 2 ! loop over all collisions going out through this channel\n";
            print OUT "\t\t\tj = ".($range_out_clust[0]-1)."+ind_out_clust(ind_quad_form(i,n),ind_quad_form(i,n+1))\n";
            print OUT "\t\t\tf(j) = f(j)+coef_quad(ind_quad_form(i,n),ind_quad_form(i,n+1),i)*&\n";
            print OUT "\t\t\t&      c(ind_quad_form(i,n))*c(ind_quad_form(i,n+1))\n";
            print OUT "\t\tend do\n";
            print OUT "\tend do\n";
        }
	}
    print OUT "\t! loop over the clusters whose concentrations are fitted (e.g. [1A]+[1A1D]=const.)\n";
	print OUT "\tdo n = 1, fitted(1,0)\n";
	if($l_j_in){
	...
	print OUT "\t\tend do\n";
	print OUT "\tend do\n";
	print OUT "\nend subroutine feval$append_to_eq_names\n\n!-----------------------------------------------------------\n\n";
	...
	print OUT "subroutine jeval$append_to_eq_names(neqn,t,c,ml,mu,dfdc,ldim,coef,ipar)\n";
	...
	print OUT ",ml,mu\n";
	print OUT "\nend subroutine jeval$append_to_eq_names\n\n!-----------------------------------------------------------\n\n";
}
...
################################################################################
	...
	print OUT "\tfor i = 1:$max_cluster_number\n";
...
if($lfortran){
	print OUT "subroutine formation$append_to_eq_names(neqn,c";
	if($lloop){
		print OUT ",j_out";
		if($l_jlim){
			print OUT ",j_lim";
	...
	print OUT ")\n";
	if($lloop && $disable_nonmonomers && $nmol_types_used>1){
		print OUT "use acdc_system$append_to_syst_names, only : monomer_indices$append_to_syst_names\n";
	}
	if($l_j_in){
		print OUT "use shared_input, only : r_j_in => rlim_for_J\n";
	...
	print OUT "\tinteger, parameter :: nclust = $max_cluster_number";
	if($lloop && $nmol_types_used>1){
		print OUT ", ij_ind_max($nmol_types_used) = (/$nmol_ranges_list/)";
	}
	if($lloop_cs){
		if($nmol_types_used==1){
			print OUT ", n_cs = $max_cluster_number";
	...
	if($lloop){
		...
		print OUT "\treal(kind(1.d0)), save :: coef_quad(nclust,nclust,$size_ind_coef)=0.d0,coef_lin($size_ind_coef,$size_ind_coef,nclust)=0.d0,source($size_C_eqn)=0.d0\n";
		print OUT "\tinteger, save :: charge(nclust)=0,ind_quad_loss($size_ind_coef,0:$ind_quad_loss[0])=0,ind_quad_form($size_ind_coef,0:$ind_quad_form[0])=0\n";
		print OUT "\tinteger, save :: ind_quad_loss_extra($size_ind_coef,0:$ind_quad_loss_extra[0])=0,ind_quad_form_extra($size_ind_coef,0:$ind_quad_form_extra[0])=0\n";
		print OUT "\tinteger, save :: ind_lin_loss($size_ind_coef,0:$ind_lin_loss[0])=0,ind_lin_form($size_ind_coef,0:$ind_lin_form[0])=0,fitted($size_C_eqn,0:$size_C_eqn)=0\n";
        if($l_save_outgoing_clust){
            print OUT "\tinteger, save :: ind_out_clust(nclust,nclust)=0\n";
	...
	if($lloop && ($nmol_types_used>1 || $l_j_in)){
		print OUT "\n";
		print OUT "\t! system size parameters\n";
		if($nmol_types_used>1){
			print OUT "\tif (nclust_inc .eq. 0) then\n";
			print OUT "\t\tcall get_molecule_numbers$append_to_eq_names(nclust_inc,indices)\n";
			print OUT "\t\tcall get_cluster_numbers$append_to_eq_names(clust_from_indices)\n";
		...
		if($l_j_in){
			print OUT "\tif (n_j_in .eq. 0) then\n";
			print OUT "\t\tdo i = 1,$nclust_included\n";
			print OUT "\t\t\tcall get_masses_and_radii$append_to_eq_names(mi,ri,i)\n";
	...
	if($lloop){
		...
		if($nmol_types_used==1 && $disable_nonmonomers){
			if($lloop_cs){
				print OUT "\tj_out = j_out+K(nclust-1,1)*c(nclust-1)*c(1)\n";
			}else{
				print OUT "\tj_out = j_out+K(nclust,1)*c(nclust)*c(1)\n";
			}
		}
		if($nmol_types_used>1 || $l_jlim || !$disable_nonmonomers || $lgrowth_for_j_tests){
			# Print a loop over the cluster sizes to go through specific collisions
			if($disable_nonmonomers){
				if($nmol_types_used>1){
					print OUT "\tdo imol = 1,$nmol_types_used\n";
					print OUT "\t\ti = n_monomers(imol)\n";
			...
			if($nmol_types_used>1){
				if($l_jlim){
					print OUT "\t\tcall get_masses_and_radii$append_to_eq_names(mi,ri,indices(i,:))\n";
				}
				if($disable_nonmonomers){
					if(!$l_j_in){
						print OUT "\t\tdo j = 1,$nclust_included\n";
				...
				print OUT "\t\t\tij_ind = indices(i,:)+indices(j,:)\n";
				print OUT "\t\t\tif (clust_from_indices($nmol_ranges_ij_ind) .eq. 0) then\n";
				print OUT "\t\t\t\tj_out = j_out+K(j,i)*c(i)*c(j)\n";
				if($lloop_boundary && $l_jlim){
					print OUT "\t\t\telse if (clust_from_indices($nmol_ranges_ij_ind) .eq. -1) then\n";
					print OUT "\t\t\t\tcycle\n";
				}
				print OUT "\t\t\tend if\n";
			}elsif($nmol_types_used==1 && !$disable_nonmonomers){
			...
			if($l_jlim){
				if($nmol_types_used>1){
					print OUT "\t\t\tcall get_masses_and_radii$append_to_eq_names(mj,rj,indices(j,:))\n";
					print OUT "\t\t\tcall get_masses_and_radii$append_to_eq_names(mk,rk,ij_ind)\n";
				}else{
					print OUT "\t\tcall get_masses_and_radii$append_to_eq_names(mi,ri,i)\n";
					if($disable_nonmonomers){
						print OUT "\t\tdo j = 1,nclust\n";
					}else{
						print OUT "\t\tdo j = i,nclust\n";
					}
					print OUT "\t\t\tcall get_masses_and_radii$append_to_eq_names(mj,rj,j)\n";
					print OUT "\t\t\tcall get_masses_and_radii$append_to_eq_names(mk,rk,i+j)\n";
				}
				print OUT "\t\t\tdo l = 1,nlim\n";
				print OUT "\t\t\t\tif (ri.lt.r_lim(l) .and. rj.lt.r_lim(l) .and. rk.ge.r_lim(l)) then\n";
				print OUT "\t\t\t\t\tj_lim(l) = j_lim(l)+K(j,i)*c(i)*c(j)\n";
				if($lloop_j_frac){
					if($nmol_types_used>1){
						print OUT "\t\t\t\t\tif (sum(indices(j,:)) .lt. sum(indices(i,:))) then\n";
						print OUT "\t\t\t\t\t\tnmin = j\n";
						print OUT "\t\t\t\t\telse\n";
						print OUT "\t\t\t\t\t\tnmin = i\n";
						print OUT "\t\t\t\t\tend if\n";
					...
					print OUT "\t\t\t\t\tj_lim_frac(l,nmin) = j_lim_frac(l,nmin)+K(j,i)*c(i)*c(j)\n";
				}
				if(!$disable_evap){
					print OUT "\t\t\t\t\t$lines{'l_evap'}";
					if($nmol_types_used>1){
			...
			if($lgrowth_for_j_tests){
				...
				if($l_jlim){
					...
					print OUT "\t\t\tdo l = 1,nlim\n";
					print OUT "\t\t\t\tif (ri.lt.r_lim(l) .and. rj.lt.r_lim(l) .and. rk.ge.r_lim(l)) then\n";
					print OUT "\t\t\t\t\tj_lim(l) = j_lim(l)+K(j,i)*c(i)*$c_growth_compound_str\n";
					if($lloop_j_frac){
						print OUT "\t\t\t\t\tnmin = min(i,j)\n";
						print OUT "\t\t\t\t\tj_lim_frac(l,nmin) = j_lim_frac(l,nmin)+K(j,i)*c(i)*$c_growth_compound_str\n";
					}
					print OUT "\t\t\t\tend if\n";
					print OUT "\t\t\tend do\n";
				}
				...
				print OUT "\t\t\tj = 3\n";
				print OUT "\t\t\tif (i+j .ge. $n_flux_out) then\n";
				print OUT "\t\t\t\tj_out = j_out+K(j,i)*c(i)*$c_growth_compound_str\n";
				print OUT "\t\t\tend if\n";
				if($l_jlim){
					print OUT "\t\t\tcall get_masses_and_radii$append_to_eq_names(mj,rj,j)\n";
					print OUT "\t\t\tcall get_masses_and_radii$append_to_eq_names(mk,rk,i+j)\n";
	...
	print OUT "\nend subroutine formation$append_to_eq_names\n\n!-----------------------------------------------------------\n\n";
}
...
if($lfortran && !$lloop){
	print OUT "subroutine initialize_parameters$append_to_eq_names(coef_quad,coef_lin,ind_quad_loss,ind_quad_form,&\n\t\t\t&ind_lin_loss,ind_lin_form,ind_quad_loss_extra,ind_quad_form_extra,&\n";
    if($l_save_outgoing_clust){
        print OUT "\t\t&\tind_out_clust,&\n";
    }
    print OUT "\t\t&\tsource,isconst,fitted";
	...
	print OUT ",ipar)\n";
	print OUT "\n\tuse acdc_simulation_setup$append_to_ext_names, only : sources_and_constants$append_to_ext_names\n\n\timplicit none\n";
	print OUT "\tinteger, parameter :: nclust=$max_cluster_number, neqn=$size_C_out\n";
	print OUT "\tlogical :: isconst($size_C_eqn)\n";
	print OUT "\treal(kind(1.d0)) :: coef_quad(nclust,nclust,$size_ind_coef),coef_lin($size_ind_coef,$size_ind_coef,nclust)\n";
	print OUT "\treal(kind(1.d0)) :: source($size_C_eqn)\n";
	print OUT "\tinteger :: ind_quad_loss($size_ind_coef,0:$ind_quad_loss[0]),ind_quad_form($size_ind_coef,0:$ind_quad_form[0]),ind_quad_loss_extra($size_ind_coef,0:$ind_quad_loss_extra[0]),ind_quad_form_extra($size_ind_coef,0:$ind_quad_form_extra[0])\n";
	print OUT "\tinteger :: ind_lin_loss($size_ind_coef,0:$ind_lin_loss[0]),ind_lin_form($size_ind_coef,0:$ind_lin_form[0])\n";
	print OUT "\tinteger :: fitted($size_C_eqn,0:$size_C_eqn)\n";
    if($l_save_outgoing_clust){
        print OUT "\tinteger :: ind_out_clust(nclust,nclust)\n"
    }
	if($l_mcmc_any){
		print OUT "\treal(kind(1.d0)) :: coef($n_mcmc_coefs)\n";
	}elsif($l_variable_parameters){
		print OUT "\treal(kind(1.d0)) :: coef($n_variable_parameters)\n";
	}
	print OUT "\tinteger :: ipar(4)";
    print OUT "\n";
	
	print OUT "\n\tcall sources_and_constants$append_to_ext_names($size_C_eqn,source,isconst,fitted)\n";
	if($include_generic_neg && $include_generic_pos){
		if($generic_positive_ions_corr ne ''){
			print OUT "\n\t! setting positive charger ion concentration to give charge balance\n";
    ...
    if($l_save_outgoing_clust){
        ...
        print OUT "\tcall get_ind_out_clust$append_to_eq_names(ind_out_clust)\n";
    }
	print OUT "\n\tcall get_rate_coefs$append_to_eq_names(coef_quad,coef_lin";
	if($l_mcmc_any || $l_variable_parameters){
		print OUT ",coef";
	}
	print OUT ")\n";
	print OUT "\nend subroutine initialize_parameters$append_to_eq_names\n\n!-----------------------------------------------------------\n\n";
	
    if($l_save_outgoing_clust){
        print OUT "subroutine get_ind_out_clust$append_to_eq_names(ind_out_clust)\n";
        print OUT "\timplicit none\n";
        print OUT "\tinteger :: ind_out_clust($max_cluster_number,$max_cluster_number)\n\n";
        print OUT "\tind_out_clust = 0\n\n";
        for($iclus=1;$iclus<=$max_cluster_number;$iclus++){
            for($jclus=$iclus;$jclus<=$max_cluster_number;$jclus++){
                if(defined $ind_out_clust[$iclus][$jclus]){
                    print OUT "\tind_out_clust($iclus,$jclus) = $ind_out_clust[$iclus][$jclus]\n";
                    print OUT "\tind_out_clust($jclus,$iclus) = $ind_out_clust[$iclus][$jclus]\n";
                }
            }
        }
        print OUT "\nend subroutine get_ind_out_clust$append_to_eq_names\n\n!-----------------------------------------------------------\n\n";
    }
	...
	print OUT "subroutine get_rate_coefs$append_to_eq_names(coef_quad,coef_lin";
	if($l_mcmc_any || $l_variable_parameters){
		print OUT ",coef";
	}
	print OUT ")\n";
	print OUT "\timplicit none\n";
	print OUT "\treal(kind(1.d0)) :: coef_quad($max_cluster_number,$max_cluster_number,$size_ind_coef),coef_lin($size_ind_coef,$size_ind_coef,$max_cluster_number)\n";
    ...
    if(!$disable_input_sink){
        print OUT ",insink($max_cluster_number)";
	}
	...
	if($l_mcmc_any){
		print OUT "\tcall get_coll$append_to_eq_names(K,coef)\n";
	}elsif( $variable_temp){
		print OUT "\tcall get_coll$append_to_eq_names(K,coef(1))\n";
	}else{
		print OUT "\tcall get_coll$append_to_eq_names(K)\n";
	}

	if($parameters_in_get_fcr_1 ne ''){
		print OUT "\tcall get_fcr$append_to_eq_names($parameters_in_get_fcr_1)\n";
	}

	if(!$disable_evap){
		if($l_mcmc_any){
			print OUT "\tcall get_evap$append_to_eq_names(E,coef)\n";
		}elsif($variable_temp){
			print OUT "\tcall get_evap$append_to_eq_names(E,K,coef(1))\n";
		}else{
			print OUT "\tcall get_evap$append_to_eq_names(E)\n";
		}
	}
	if($l_use_get_losses){
		$line = '';
        ...
        if(!$disable_coag_sinks && $coag_terms =~ m/^external$/i){
            if($variable_temp){
                $line = "$line,coef(1)";
            }else{
		...
		print OUT "\tcall get_losses$append_to_eq_names($line)\n";
	}
	if($variable_cs){
		if($one_cs_for_all){
	...
	print OUT "\nend subroutine get_rate_coefs$append_to_eq_names\n\n!-----------------------------------------------------------\n\n";
}	
...
if($lloop){
	if($nmol_types_used>1){
		print OUT "subroutine get_masses_and_radii$append_to_eq_names(m,r,i_ind)\n";
	}else{
		print OUT "subroutine get_masses_and_radii$append_to_eq_names(m,r,i)\n";
	}
	print OUT "\timplicit none\n";
    # Clumsy to have these hard-coded here...
	...
	for($imol=0;$imol<$nmol_types;$imol++){
		if($n_max_type[$imol]>0){
			...
			$string1 .= "real(i_ind($i),kind=kind(1.d0))*m$i+";
			$string2 .= "real(i_ind($i),kind=kind(1.d0))*monvol$i+";
			if($j==2 && $imol<$nmol_types-1){
				$j = 0;
				$string1 .= "&\n\t\t&";
				$string2 .= "&\n\t\t&";
			}
	...
	if($nmol_types_used>1){
		...
		print OUT "\tinteger :: i\n\n";
		$string1 = "real(i,kind=kind(1.d0))*m1";
		$string2 = "real(i,kind=kind(1.d0))*monvol1";
	}
	print OUT "\tm = $string1\n";
	print OUT "\tr = (3.d0/4.d0/pi*$string2)**(1.d0/3.d0)\n\n";
	print OUT "\nend subroutine get_masses_and_radii$append_to_eq_names\n\n!-----------------------------------------------------------\n\n";
	
	if($nmol_types_used>1){
		print OUT "subroutine get_molecule_numbers$append_to_eq_names(nclust_inc,indices)\n";
		print OUT "\timplicit none\n";
		print OUT "\tinteger, parameter :: nclust = $max_cluster_number\n";
		print OUT "\tinteger :: n, $nmol_ranges_ind, nclust_inc, indices(nclust,$nmol_types_used), clust_from_indices($nmol_comp_list)\n\n";
		
		print OUT "\tnclust_inc = 0\n";
		print OUT "\tindices = 0\n\n";
		print OUT "\tcall get_cluster_numbers$append_to_eq_names(clust_from_indices)\n\n";
		print OUT "\tn = 0\n";
		...
		print OUT $str_temp."if (clust_from_indices($nmol_ranges_ind) .gt. 0) then\n";
		$str_temp2 = $str_temp."end if\n".$str_temp2;
		$str_temp .= "\t";
		print OUT $str_temp."n = n+1\n";
		print OUT $str_temp."indices(clust_from_indices($nmol_ranges_ind),:) = (/$nmol_ranges_ind/)\n";
		print OUT $str_temp2;
		print OUT "\n\tnclust_inc = n\n";
		
		print OUT "\nend subroutine get_molecule_numbers$append_to_eq_names\n\n!-----------------------------------------------------------\n\n";
		
		print OUT "subroutine get_cluster_numbers$append_to_eq_names(clust_from_indices)\n";
		if($lloop_restrict){
			print OUT "use restriction_criteria$append_to_ext_names\n";
			print "\n---------------------------------------------------------\n";
			print "\tYou will need the module restriction_criteria$append_to_ext_names\n\tin order to compile and run the code.\n";
			print "---------------------------------------------------------\n\n";
		}
		print OUT "\timplicit none\n";
		if($lloop_cs && $lloop_restrict){
			print OUT "\tinteger, parameter :: ij_ind_cs($nmol_types_used) = (/$nmol_cs_list/)\n";
			print OUT "\tlogical :: lfound\n";
		}
		print OUT "\tinteger :: n, $nmol_ranges_ind, ij_ind($nmol_types_used), clust_from_indices($nmol_comp_list)";
		if($lloop_restrict){
			print OUT ", valid";
		}
		print OUT "\n\n";
		print OUT "\tclust_from_indices = 0\n";
		if($lloop_cs && $lloop_restrict){
			print OUT "\tlfound = .false.\n";
		}
		print OUT "\n\tn = 0\n";
		...
		for($imol=1;$imol<=$nmol_types_used;$imol++){
			...
			if(!$lloop_boundary){
				...
				# Loop over all collision products, as some of them may not be valid
				print OUT $str_temp."do i$imol = 0,".(2*$nmol_ranges_array[$imol])."\n";
			}
			$str_temp2 = $str_temp."end do\n".$str_temp2;
			$str_temp .= "\t";
		}
		# ... but they can't all be 0 ...
		$string = $nmol_ranges_ind;
		$string =~ s/,\s*/\+/g;
		...
		$string = "";
		if($lloop_restrict){
			print OUT $str_temp."call check_validity$append_to_ext_names((/$nmol_ranges_ind/),valid)\n";
			print OUT $str_temp."if (valid .eq. 1) then\n";
			$string = "\t";
		}
		print OUT $str_temp.$string."n = n+1\n";
		print OUT $str_temp.$string."clust_from_indices($nmol_ranges_ind) = n\n";
		if($lloop_restrict){
			if($lloop_cs){
				print OUT $str_temp."\tif (all((/$nmol_ranges_ind/) .eq. ij_ind_cs)) then\n";
				print OUT $str_temp."\t\tlfound = .true.\n";
				print OUT $str_temp."\tend if\n";
			}
			print OUT $str_temp."else\n";
			print OUT $str_temp."\tclust_from_indices($nmol_ranges_ind) = valid\n";
			print OUT $str_temp."end if\n";
		}
		print OUT $str_temp2;
		if($lloop_cs && $lloop_restrict){
			print OUT "\t! sink cluster, if it's not yet included in the set\n";
			print OUT "\tif (.not. lfound) then\n";
			print OUT "\t\tn = n+1\n";
			print OUT "\t\tij_ind = ij_ind_cs\n";
			print OUT "\t\tclust_from_indices($nmol_ranges_ij_ind) = n\n";
			print OUT "\tend if\n";
		}

		print OUT "\nend subroutine get_cluster_numbers$append_to_eq_names\n\n!-----------------------------------------------------------\n\n";
...
if($lfortran){
	if($l_use_get_losses){
        ...
        if(!$disable_input_sink){
            ...
            $lines{'inp'} .= 'insink';
            if($lloop){
                $lines{'def'} .= "insink(nrates)";
            }else{
                $lines{'def'} .= "insink($max_cluster_number)";
            }
		}
        if(!$disable_coag_sinks && $coag_terms =~ m/^external$/i){
            $lines{'inp'} .= ",temperature";
            $lines{'def'} .= ",temperature";
        }
        $str_temp = "";
        if(!$disable_input_sink && $module_insink ne ''){
            $str_temp = "\n\tuse $module_insink, only : $routine_insink\n";
        }
		if(!$lloop){
			print OUT "subroutine get_losses$append_to_eq_names($lines{'inp'})\n";
            if(!$disable_coag_sinks && $coag_terms =~ m/^external$/i){
                print OUT "\n\tuse acdc_simulation_setup$append_to_ext_names, only : get_scav_parameters$append_to_ext_names\n";
            }
            print OUT $str_temp;
			print OUT "\n\timplicit none\n\treal(kind(1.d0)) :: $lines{'def'}\n\n";
		}else{
			print OUT "subroutine get_losses$append_to_eq_names(nrates,loss";
			if($variable_cs){
				print OUT ",coef";
			}
			print OUT ")\n";
			if(!$disable_coag_sinks && $coag_terms =~ m/^bg_loss$/i){
				...
				#if($lloop_j_tests){
				print OUT "\n\tuse shared_input, only : cbg => bg_concentration\n";
			...
			if((!$disable_coag_sinks && !$one_cs_for_all) || ((!$disable_wall_terms || ($l_mcmc_any && $ncoef_wl>0)) && !$one_wl_for_all)){
				print OUT "(nrates), m, r";
				if(!$disable_wall_terms && ($wall_terms =~ m/^cloud4_JK$/i || $wall_terms =~ m/^cloud4_AK$/i)){
					print OUT ", d, sc";
				}
				if(!$disable_coag_sinks){
					if($coag_terms =~ m/^exp_loss$/i){
			...
			print OUT "\n";
			print OUT "\tinteger, parameter :: nclust = $max_cluster_number\n";
			print OUT "\tinteger :: nrates, i\n";
			if($nmol_types_used>1){
				print OUT "\tinteger, save :: nclust_inc=0, indices(nclust,$nmol_types_used)";
				if($#cs_only>=0 || $#insink_only>=0){
					print OUT ", clust_from_indices($nmol_comp_list)";
				}
				print OUT "\n\n";
				print OUT "\tif (nclust_inc .eq. 0) then\n";
...
################################################################################
	...
	if($coag_terms =~ m/^exp_loss$/i){
		...
		if(!$lloop){
            ...
            if($exp_loss_ref_size ne ''){
                $r1=$exp_loss_ref_size;
			}elsif($exp_loss_ref_cluster ne ''){
			...
			for($iclus=1;$iclus<=$max_cluster_number;$iclus++){
				...
				for($ihydr=0;$ihydr<=$#{ $distr[$iclus] };$ihydr++){
					if(defined $distr[$iclus][$ihydr]){
						if($ihydr==0){
							$temp_label=$label[$iclus];
						}else{
							$temp_label=$label[$iclus].$ihydr.$name_water;
						}
			...
			$cs_coef_hydr[0][0] = "\t\tcs(i) = ";
			if(!$variable_cs){
				$cs_coef_hydr[0][0] .= $exp_loss_coefficient."*";
			}
			$cs_coef_hydr[0][0] .= "(r/r_ref)**$exp_loss_exponent\n";
			
			# Find the reference size
			if($exp_loss_ref_size ne ''){
                $exp_loss_ref_size = sprintf('%.14e',$exp_loss_ref_size);
                $exp_loss_ref_size =~ s/e/d/;
                print OUT "\t! reference radius given as input\n";
                print OUT "\tr_ref = $exp_loss_ref_size\n\n";
			}elsif($exp_loss_ref_cluster ne ''){
				...
				if($nmol_types_used>1){
					$i = "(/".join(',',@temp_array[@mol_types_used[1..$nmol_types_used]])."/)";
				}else{
					$imol = 1;
				...
				for($imol=1;$imol<=$nmol_types_used;$imol++){
					if($molecule_name[$mol_types_used[$imol]] eq $default_monomer){
						$lfound_default = 1;
						@temp_array = (0) x $nmol_types_used;
						$temp_array[$imol-1] = 1;
						$i = "(/".join(',',@temp_array)."/)";
					}
				}
            ...
            if($exp_loss_ref_size eq ''){
                print OUT "\tcall get_masses_and_radii$append_to_eq_names(m,r_ref,$i)\n\n";
            }
...
################################################################################
		...
		if($variable_temp){
			die "\nVariable temperature not implemented for the bg_loss option.\n\n";
		}
		...
		if(!$lloop){
			...
			$string4 =~ s/e/d/;
			print OUT "\tDbg = $string2/(2.d0*rbg)*((5.d0+4.d0*($string1/rbg)+&\n\t\t\t\t& 6.d0*($string1/rbg)**2.d0+18.d0*($string1/rbg)**3)/&\n\t\t\t\t& (5.d0-($string1/rbg)+(8.d0+$string4)*($string1/rbg)**2.d0))\n";
			...
			$cs_coef_hydr[0][0] .= "$str_temp\t\tD = $string2/(2.d0*r)*((5.d0+4.d0*($string1/r)+&\n\t\t\t\t& 6.d0*($string1/r)**2.d0+18.d0*($string1/r)**3)/&\n\t\t\t\t& (5.d0-($string1/r)+(8.d0+$string4)*($string1/r)**2.d0))\n";
			$cs_coef_hydr[0][0] .= "$str_temp\t\tvth = sqrt($string3/m)\n";
			$cs_coef_hydr[0][0] .= "$str_temp\t\tKn = 2.d0*(D+Dbg)/(sqrt(vth**2.d0+vthbg**2.d0)*(r+rbg))\n";
            $cs_coef_hydr[0][0] .= "$str_temp\t\tcs(i) = 4.d0*$string4*(r+rbg)*(D+Dbg)*(1.d0+Kn)/(1.d0+2.d0*Kn*(1.d0+Kn))";
			if(!$variable_cs){
				$cs_coef_hydr[0][0] .= "*cbg";
			}
			$cs_coef_hydr[0][0] .= "\n";
...
################################################################################
    ...
    }elsif($coag_terms =~ m/^external$/i){
        # Print out the call to the external routine
        # Possible special coagulation losses for specific clusters are taken into account below
        print OUT "\t! cs obtained from an external routine\n";
        print OUT "\tcall get_scav_parameters$append_to_ext_names(temperature=temperature,cs_per_clust=cs)\n\n";
...
################################################################################
		...
		if($#cs_only>=0){
			if($lfortran){
				$cs_term_temp = sprintf('%.14e',$coag_terms);
			}else{
				$cs_term_temp = $coag_terms;
			}
		}else{
...
# If we have special losses for some clusters, get them here
	if($#cs_only>=0){
		if(!$lloop){
			...
			for($icol=0;$icol<=$#cs_only;$icol++){
				@columns=split(',',$cs_only[$icol]);
				if ($#columns!=1){
					die "Can't understand the special coagulation loss $cs_only[$icol] - the format should be 'clust,number'.\n";
				}
				($iclus,$nwaters)=&get_corresponding_dry($columns[0]);
				if($iclus eq ''){
					die "Cluster $columns[0] from the special coagulation loss $cs_only[$icol] does not exist.\n";
				}
				$cs_only_hydr[$iclus][$nwaters] = sprintf('%.14e',$columns[1]);
			}
			...
			$icol = -1;
			for($iclus=1;$iclus<=$max_cluster_number;$iclus++){
				next if ($#{ $cs_only_hydr[$iclus] }==-1);
				$icol += 1;
				$cs_only[$icol][0] = $iclus;
				$cs_only[$icol][1] = 0;
				for($ihydr=0;$ihydr<=$#{ $distr[$iclus] };$ihydr++){
					if(defined $distr[$iclus][$ihydr]){
						if(defined $cs_only_hydr[$iclus][$ihydr]){
							$cs_only[$icol][1] += $distr[$iclus][$ihydr]*$cs_only_hydr[$iclus][$ihydr];
						}else{
							if($#{ $cs_only_hydr[$iclus] }==0){
								print "You have defined special coagulation losses for $label[$iclus] but not its hydrates -> using the given value for the hydrate average.\n";
								$cs_only[$icol][1] = $cs_only_hydr[$iclus][0];
								last;
							}else{
								print "You have defined special coagulation losses for some but not all hydrates of $label[$iclus] -> using the standard coagulation losses for the others.\n";
								if($one_cs_for_almost_all){
									$cs_only[$icol][1] += $distr[$iclus][$ihydr]*$cs_term_temp;
								}else{
									$cs_only[$icol][1] += $distr[$iclus][$ihydr]*$cs_coef_hydr[$iclus][$ihydr];
								}
							}
						}
					}
			...
			for($icol=0;$icol<=$#cs_only;$icol++){
				...
				$valid = 1;
				($string1,$string2) = &determine_cluster_composition($columns[0]);
				@temp_array=@$string1;
				if($string2 ne '' || sum(@temp_array)>sum(@temp_array[@mol_types_used[1..$nmol_types_used]])){
					# cluster contains molecule types not included in the system
					$valid = 0;
				...
				if(!$valid){
					die "Special coagulation loss cluster $columns[0] is not a valid cluster\n";
				}
				if($nmol_types_used>1){
					$iclus = "clust_from_indices(".join(',',@temp_array[@mol_types_used[1..$nmol_types_used]]).")";
				}else{
				...
				$string1 = sprintf('%.14e',$columns[1]);
				$string1 =~ s/e/d/;
				$lines{'loss_only'} .= "\tcs($iclus) = $string1\n";
			}
		}
	}
	if(!$lloop && $one_cs_for_almost_all){
		# If we have a constant coagulation loss for most clusters and a special value for others,
		# print the coefficients here.
		for($iclus=1;$iclus<=$max_cluster_number;$iclus++){
			$str_temp = $cs_term_temp;
			for($icol=0;$icol<=$#cs_only;$icol++){
				if($cs_only[$icol][0]==$iclus){
					$str_temp = sprintf('%.14e',$cs_only[$icol][1]);
					if(!&is_pos_number($str_temp)){
						die "\nCan't understand coagulation loss definition $str_temp for cluster $label[$iclus]\n";
					}elsif(!$lsilent){
						print "Using special coagulation loss $str_temp for cluster $label[$iclus].\n";
					}
					last;
				}
			}
			if($lfortran){
				$str_temp =~ s/e/d/;
				print OUT "\tcs($iclus) = $str_temp\t! coagulation loss of $label[$iclus]\n";
			}else{
				print CS "CS($iclus) = $str_temp;\t% coagulation loss of $label[$iclus]\n";
			}
		}
...
################################################################################
	...
	}elsif(!$lhydrate_average && !$one_cs_for_all){
		if(!$lloop){
			for($iclus=1;$iclus<=$max_cluster_number;$iclus++){
                $cs_term_temp="";
                if($coag_terms !~ m/^external$/i){
				...
				if($#cs_only>=0){
					for($icol=0;$icol<=$#cs_only;$icol++){
						if($cs_only[$icol][0]==$iclus){
							$cs_term_temp = sprintf('%.14e',$cs_only[$icol][1]);
							if(!&is_pos_number($cs_term_temp)){
								die "\nCan't understand coagulation loss definition $cs_term_temp for cluster $label[$iclus]\n";
							}elsif(!$lsilent){
								print "Using special coagulation loss $cs_term_temp for cluster $label[$iclus].\n";
							}
							last;
						}
					}
			...
			print OUT "\tdo i = 1,$nclust_included\n";
			if($nmol_types_used>1){
				print OUT "\t\tcall get_masses_and_radii$append_to_eq_names(m,r,indices(i,:))\n";
			}else{
				print OUT "\t\tcall get_masses_and_radii$append_to_eq_names(m,r,i)\n";
			}
...
################################################################################	
		...
		for($iclus=1;$iclus<=$max_cluster_number;$iclus++){
			...
			if($#cs_only>=0){
				for($icol=0;$icol<=$#cs_only;$icol++){
					if($cs_only[$icol][0]==$iclus){
						$cs_term_temp = sprintf('%.14e',$cs_only[$icol][1]);
						$str_temp=sprintf('%.14e',$cs_only_hydr[$iclus][0]);
						$str_temp2=" (using a special coagulation loss)";
						if(!&is_pos_number($cs_term_temp)){
							die "\nCan't understand coagulation loss definition $cs_term_temp for cluster $label[$iclus]\n";
						}elsif(!$lsilent){
							print "Using special coagulation loss $cs_term_temp for cluster $label[$iclus].\n";
            ...
            if($cs_term_temp ne ''){
                if($lfortran){
                    ...
                    $str_temp =~ s/e/d/;
                    if($lhas_hydr[$iclus]){
                        $string="$cs_term_temp\t! coagulation loss of $label[$iclus], dry value $str_temp$str_temp2";
                    }else{
                        $string="$cs_term_temp\t! coagulation loss of $label[$iclus], no hydrates$str_temp2";
                    }
                    print OUT "\tcs($iclus) = $string\n";
                }else{
                    if($lhas_hydr[$iclus]){
                        $string="CS($iclus) = $cs_term_temp;\t% coagulation loss of $label[$iclus], dry value $str_temp$str_temp2";
                    }else{
                        $string="CS($iclus) = $cs_term_temp;\t% coagulation loss of $label[$iclus], no hydrates$str_temp2";
                    }
                    print CS "$string\n";
                }
            }
	...
	# Adding to the loop mode the values that are not included in the do-loops
	if($lloop){
		if($#cs_only>=0){
			print OUT $lines{'loss_only'}."\n";
		}
		if($variable_cs){
			if($one_cs_for_all){
				print OUT "\tcs = coef(" . ($variable_temp+1) . ")\n\n";
...
##############################################################################################
	...
	}elsif($wall_terms =~ m/^cloud4_JK$/i){
	# CLOUD wall losses (parameterization from experiments)
		
		if($variable_temp){
			die "\nVariable temperature not implemented for the cloud4_JK option.\n\n";
		}
...
#################################################################################
	...
	}elsif($wall_terms =~ m/^cloud4_AK$/i){
	# CLOUD wall losses (parameterization from experiments)
		
		if($variable_temp){
			die "\nVariable temperature not implemented for the cloud4_AK option.\n\n";
		}
...
###################################################################################################################
	...
	}elsif($wall_terms =~ m/^cloud4_JA$/i){
	# CLOUD wall losses (parameterization from experiments)
		if(!$lloop){
			for($iclus=1;$iclus<=$max_cluster_number;$iclus++){
				# loss coefficients for the dry cluster and possible hydrates
...
##########################################################################################################################
	...
	}elsif($wall_terms =~ m/^cloud4_simple$/i){
	# CLOUD wall losses (parameterization from experiments, very simple)
		if(!$lloop){
			for($iclus=1;$iclus<=$max_cluster_number;$iclus++){
				# loss coefficients for the dry cluster and possible hydrates
				for($ihydr=0;$ihydr<=$#{ $distr[$iclus] };$ihydr++){
					if(defined $distr[$iclus][$ihydr]){
						...
						# WL factor proportional to 1/mobility diameter; mobility diameter = mass diameter + 0.3 nm (no mass dependence)
						$wl_term_temp = 1.0/(2.0*$radius+0.3e-9);
						$wl_term_temp *= 1.0e-12;
						$wl_coef_hydr[$iclus][$ihydr] = $wl_term_temp;
					}
				}
			}
		}else{
			$wl_coef_hydr[0][0] = "\t\twl(i) = 1.d-12/(2.d0*r+0.3d-9)\n";
		}
...
###############################################################################################################
		...
		if($variable_temp){
			die "\nVariable temperature not implemented for the diffusion loss option.\n\n";
		}
		...
		if(!$lloop){
			...
			$wl_term_0 =~ s/e/d/;
			$wl_coef_hydr[0][0] = "\t\twl(i) = 1.d0/(r+$radius_N2)**2.d0*sqrt(1.d0/m+1.d0/$mass_N2)/$D0_factor*$wl_term_0\n";
		}
			...
			for($iclus=1;$iclus<=$max_cluster_number;$iclus++){
				...
				for($icol=0;$icol<=$#wl_only;$icol++){
					if($wl_only[$icol][0]==$iclus){
						$str_temp = sprintf('%.14e',$wl_only[$icol][1]);
						if(!&is_pos_number($str_temp)){
		...
		if(!$lloop){
			for($iclus=1;$iclus<=$max_cluster_number;$iclus++){
				...
				if($#wl_only>=0){
					for($icol=0;$icol<=$#wl_only;$icol++){
						if($wl_only[$icol][0]==$iclus){
							$wl_term_temp = sprintf('%.14e',$wl_only[$icol][1]);
		...
		for($iclus=1;$iclus<=$max_cluster_number;$iclus++){
			...
			if($#wl_only>=0){
				for($icol=0;$icol<=$#wl_only;$icol++){
					if($wl_only[$icol][0]==$iclus){
						$wl_term_temp = sprintf('%.14e',$wl_only[$icol][1]);
						$str_temp=sprintf('%.14e',$wl_only_hydr[$iclus][0]);
						$str_temp2=" (using a special wall loss)";
						if(!&is_pos_number($wl_term_temp)){
							die "\nCan't understand wall loss definition $wl_term_temp for cluster $label[$iclus]\n";
						}elsif(!$lsilent){
							print "Using special wall loss $wl_term_temp for cluster $label[$iclus].\n";
			...
			if($lfortran){
				...
				$str_temp =~ s/e/d/;
				if($lhas_hydr[$iclus]){
					$string="$wl_term_temp\t! wall loss of $label[$iclus], dry value $str_temp$str_temp2";
				}else{
					$string="$wl_term_temp\t! wall loss of $label[$iclus], no hydrates$str_temp2";
				}
				if($l_special_mcmc_wl[$iclus]){
					# If this is an MCMC coefficient, save the true value to be printed later ...
					$icoef_wl++;
				...
				if($lhas_hydr[$iclus]){
					$string="WL($iclus) = $wl_term_temp;\t% wall loss of $label[$iclus], dry value $str_temp$str_temp2";
				}else{
					$string="WL($iclus) = $wl_term_temp;\t% wall loss of $label[$iclus], no hydrates$str_temp2";
				}
				print WL "$string\n";
			}
		}
...
if(!$disable_input_sink){
    ...
    # Print out the call to the external routine
    # Possible special losses for specific clusters are taken into account below
    print OUT "\n\t! Input sink obtained from an external routine\n";
    print OUT "\tcall $routine_insink(insink=insink)\n\n";
...
# If we have special losses for some clusters, get them here
	if($#insink_only>=0){
		if(!$lloop){
            @temp_array = @insink_only;
            @insink_only = ();
            for($icol=0;$icol<=$#temp_array;$icol++){
				@columns=split(',',$temp_array[$icol]);
				if ($#columns!=1){
					die "Can't understand the special input sink $temp_array[$icol] - the format should be 'clust,number'.\n";
				}
				($iclus,$nwaters)=&get_corresponding_dry($columns[0]);
				if($iclus eq ''){
					die "Cluster $columns[0] from the special input sink $temp_array[$icol] does not exist.\n";
				}elsif($nwaters > 0){
                    die "Cluster $columns[0] from the special input sink $temp_array[$icol]: not possible to include hydrates for the input sink.\n";
                }
                $str_temp = sprintf('%.14e',$columns[1]);
                if(!&is_pos_number($str_temp)){
                    die "\nCan't understand input sink definition $str_temp for cluster $label[$iclus]\n";
                }elsif(!$lsilent){
                    print "Using special input sink $str_temp for cluster $label[$iclus].\n";
                }
                $insink_only[$icol][0] = $iclus;
                $insink_only[$icol][1] = $str_temp;
                if($lfortran){
                    $str_temp =~ s/e/d/;
                    print OUT "\tinsink($iclus) = $str_temp\t! input sink of $label[$iclus]\n";
                }
			}
		}else{
			for($icol=0;$icol<=$#insink_only;$icol++){
				@columns=split(',',$insink_only[$icol]);
				if ($#columns!=1){
					die "Can't understand the special input sink $insink_only[$icol] - the format should be 'clust,number'.\n";
				}
				$valid = 1;
				($string1,$string2) = &determine_cluster_composition($columns[0]);
				@temp_array=@$string1;
				...
				if(!$valid){
					die "Special input sink cluster $columns[0] is not a valid cluster\n";
				}
				if($nmol_types_used>1){
					$iclus = "clust_from_indices(".join(',',@temp_array[@mol_types_used[1..$nmol_types_used]]).")";
				}else{
				...
				$string1 = sprintf('%.14e',$columns[1]);
				$string1 =~ s/e/d/;
                print OUT "\tinsink($iclus) = $string1\n";
			}
            print OUT "\n";
		}
	}
...
if($lfortran && $l_use_get_losses){
	...
	print OUT "\nend subroutine get_losses$append_to_eq_names\n\n!-----------------------------------------------------------\n\n";
}
...
if($lfortran){
	...
	print OUT "subroutine get_coll$append_to_eq_names(";
	if($lloop){
		print OUT "nrates,";
	}
	print OUT "K";
	...
	print OUT "\tinteger, parameter :: nclust = $max_cluster_number\n";
	if($lloop){
		print OUT "\treal(kind(1.d0)) :: K(nrates,nrates)";
	}else{
...
if(($sticking_factor[0][0] == 1) && ($sticking_factor[1][0] == 1) && ($sticking_factor_file_name eq '')){
	...
	if($sticking_factor_file_name ne ''){
        if(!$lsilent){
            print "\nSticking factor file: $sticking_factor_file_name\n";
        }
...
if($include_ion_terms){
	# The Su73 and Su82 ion collision parameterizations require dipole moments and polarizabilities
	...
	if($ion_coll_method =~ /Su73/i || $ion_coll_method =~ /Su82/i && !(!$dip_file_name && $l_coll_factor_mcmc_n_i && ($l_coll_factor_mcmc_n_g || (!$include_generic_neg && !$include_generic_pos)))){
		...
		if(!$disable_evap || $lhydrate_average){
			# Do they have similar names?
			if(length($dip_file_name)>8 && length($free_energy_file_name)>8 && (substr($dip_file_name,-8) ne substr($free_energy_file_name,-8))){
				# Were they last modified within an hour of each other?
				if(abs((stat($dip_file_name))[9]-(stat($free_energy_file_name))[9])/3600>1){
					print "!!!!! Warning: do your energy and dipole files belong together? !!!!!\n";
				}
			}
		}
...
################################################################################
			...
			if($loop_coll_coef =~ m/^hard_spheres$/i){
				...
				$string1 =~ s/e/d/;
				if(!$variable_temp){
					print OUT "\t\t\tK(i,j) = sqrt($string1*(1.d0/mi+1.d0/mj))*(ri+rj)**2.d0\n";
				}else{
					print OUT "\t\t\tK(i,j) = sqrt($string1*temperature*(1.d0/mi+1.d0/mj))*(ri+rj)**2.d0\n";
				}
			}elsif($loop_coll_coef =~ m/^Dahneke$/i){
				if($variable_temp){
					die "\nVariable temperature not implemented for the Dahneke parameterization.\n\n";
				}
...
#print "mass($label[$iclus]$ihydr" . "$name_water)=$mass1\n";
#print "mass($label[$jclus]$jhydr" . "$name_water)=$mass2\n";
#print "$label[$jclus]$jhydr" . "$name_water: $lhas_hydr[$jclus],$#distr_temp2, @distr_temp2\n\n";
...
################################################################################
							...
							if(!defined($dip_hydr[$nclus][$nhydr])){
								...
								$temp_val =~ s/e/d/;
								$fcr_temp .= "*(($temp_val/sqrt(temperature)+0.5090d0)**2.d0/10.526d0+0.9754d0)";
								...
								$temp_val = sprintf('%.14e',(50.*$dip_hydr[$nclus][$nhydr])**2/(2.*$pol_hydr[$nclus][$nhydr]*$boltz*1.e23));
                                $fcr_temp = "max(sign(temperature-$temp_val),0)";
								$str_temp = "max(sign($temp_val-temperature),0)";
								$temp_val = sprintf('%.14e',100.*$dip_hydr[$nclus][$nhydr]/sqrt(2.*$pol_hydr[$nclus][$nhydr]*$boltz*1.e23));
			...
			if($lhydrate_average){
				if($lhas_hydr[$iclus] || $lhas_hydr[$jclus]){
					if($lneutral_cluster[$iclus]+$lneutral_cluster[$jclus] == 1){
                        ...
                        if(defined $distr[$iclus][0] && defined $distr[$jclus][0]){
                            $hydr_string = ", dry value: " . sprintf('%.14e',$coll_coef_hydr[$iclus][$jclus][0][0]);
                        }else{
                            $hydr_string = ", dry value undefined";
                        }
					}
...
if($lfortran){
	print OUT "\nend subroutine get_coll$append_to_eq_names\n\n!-----------------------------------------------------------\n\n";
}else{
	if($variable_temp){
		print COLL "\nend";
	}
...
if(!$disable_evap){
	...
	if($lfortran){
		print OUT "subroutine get_evap$append_to_eq_names(";
		if($lloop){
			print OUT "nrates,";
		}
		print OUT "E";
		...
		print OUT ")\n";
		if($lloop && $nmol_types_used>1 && $loop_evap_coef =~ m/^Kelvin$/i){
			print OUT "use acdc_system$append_to_syst_names, only : monomer_indices$append_to_syst_names\n";
		}
		if($lhs_tests){
			print OUT "use acdc_hsdata$append_to_ext_names, only : get_hsdata$append_to_ext_names\n";
		}
		print OUT "\timplicit none\n";
		if(!$lloop){
...
################################################################################
	...
	if($lloop){
		...
		print OUT "\n";
		print OUT "\treal(kind(1.d0)) :: E(nrates,";
		if($disable_nonmonomer_evaps && $nmol_types_used==1){
			print OUT "1)";
		}else{
			print OUT "nrates)";
		}
		print OUT ", K(nrates,nrates), temperature\n";
		if($nmol_types_used>1){
			print OUT "\tinteger, save :: nclust_inc=0, indices(nclust,$nmol_types_used), clust_from_indices($nmol_comp_list)\n";
		}
		print OUT "\treal(kind(1.d0)) :: m, r, xmol\n";
		print OUT "\tinteger :: nrates, ij_ind($nmol_types_used), i, j, ij, imol";
		if($lhs_tests){
			print OUT ", ind(2)\n";
			print OUT "\tlogical, save, allocatable :: lhs(:)\n";
			print OUT "\treal(kind(1.d0)), save, allocatable :: deltah(:), deltas(:)";
		}
		print OUT "\n";
...
################################################################################
		...
		if($loop_evap_coef =~ m/^DeltaG$/i){
			...
			if($variable_temp){
				print "\nVariable temperature not implemented in the cluster_energies input.\n\n";
			}else{
				print OUT "\tcall get_coll$append_to_eq_names(nrates,K)\n";
			}
			
			if($disable_nonmonomer_evaps && $nmol_types_used==1){
				if($lloop_cs){
					print OUT "\tdo i = 1,nclust-2\n";
				}else{
					print OUT "\tdo i = 1,nclust-1\n";
				}
				if($rlim_no_evap !~ /^\s*$/){
					print OUT "\t\tcall get_masses_and_radii$append_to_eq_names(m,r,i+1)\n";
					print OUT "\t\tif(r .ge. $rlim_no_evap_str) then\n";
					print OUT "\t\t\texit\n";
					print OUT "\t\tend if\n";
				}
				print OUT "\t\tif (i .eq. 1) then\n";
				print OUT "\t\t\tcall cluster_energies$append_to_ext_names(gi,p,1,i)\n";
				print OUT "\t\t\tgj = gi\n";
				print OUT "\t\telse\n";
				print OUT "\t\t\tgi = gij\n";
				print OUT "\t\tend if\n";
				print OUT "\t\tcall cluster_energies$append_to_ext_names(gij,p,1,i+1)\n";
				print OUT "\t\tE(i,1) = K(i,1)*p/$string1*exp((gij-gi-gj)*$string2)\n";
				print OUT "\t\tif (i .eq. 1) then\n";
				print OUT "\t\t\tE(i,1) = .5d0*E(i,1)\n";
				print OUT "\t\tend if\n";
				print OUT "\tend do\n\n";
			}else{
				print OUT "\tdo i = 1,$nclust_included\n";
				if($nmol_types_used>1){
					print OUT "\t\tcall cluster_energies$append_to_ext_names(gi,p,$nmol_types_used,indices(i,:))\n";
				}else{
					print OUT "\t\tcall cluster_energies$append_to_ext_names(gi,p,1,i)\n";
				}
				print OUT "\t\tdo j = i,$nclust_included\n";
				if($nmol_types_used>1){
					print OUT "\t\t\tij_ind = indices(i,:)+indices(j,:)\n";
					if($rlim_no_evap !~ /^\s*$/){
						print OUT "\t\t\tcall get_masses_and_radii$append_to_eq_names(m,r,ij_ind)\n";
						print OUT "\t\t\tif(r .ge. $rlim_no_evap_str) then\n";
						print OUT "\t\t\t\tcycle\n";
						print OUT "\t\t\tend if\n";
					}
					print OUT "\t\t\tif(clust_from_indices($nmol_ranges_ij_ind).gt.0";
					if($lloop_cs){
						print OUT " .and. .not.all(ij_ind .eq. ij_ind_cs)";
					}
					print OUT ") then\n";
					print OUT "\t\t\t\tij = clust_from_indices($nmol_ranges_ij_ind)\n";
					print OUT "\t\t\t\tcall cluster_energies$append_to_ext_names(gj,p,$nmol_types_used,indices(j,:))\n";
					print OUT "\t\t\t\tcall cluster_energies$append_to_ext_names(gij,p,$nmol_types_used,ij_ind)\n";
				}else{
					print OUT "\t\t\tij=i+j\n";
					if($rlim_no_evap !~ /^\s*$/){
						print OUT "\t\tcall get_masses_and_radii$append_to_eq_names(m,r,ij)\n";
						print OUT "\t\tif(r .ge. $rlim_no_evap_str) then\n";
						print OUT "\t\t\texit\n";
						print OUT "\t\tend if\n";
					}
					print OUT "\t\t\tif(ij .le. nclust) then\n";
					print OUT "\t\t\t\tcall cluster_energies$append_to_ext_names(gj,p,1,j)\n";
					print OUT "\t\t\t\tcall cluster_energies$append_to_ext_names(gij,p,1,ij)\n";
				}
...
################################################################################
			...
			print OUT "\tinteger, save :: n_monomers($nmol_types_used)\n";
			...
			if($nmol_types_used>1){
				...
				print OUT "\t\tcall monomer_indices$append_to_syst_names(n_monomers)\n";
				if($lhs_tests){
					print OUT "\t\tallocate(lhs(nrates))\n\t\tallocate(deltah(nrates))\n\t\tallocate(deltas(nrates))\n";
					print OUT "\t\tdo i = 1,nclust_inc\n";
					@temp_array = ('A','N');
					for($i=0;$i<=$#temp_array;$i++){
						for($imol=1;$imol<=$nmol_types_used;$imol++){
							if($molecule_name[$mol_types_used[$imol]] eq $temp_array[$i]){
								print OUT "\t\t\tind(".($i+1).") = indices(i,$imol)\n";
								last;
							}
						}
					}
					print OUT "\t\t\tcall get_hsdata$append_to_ext_names(ind,lhs(i),deltah(i),deltas(i))\n";
					print OUT "\t\tend do\n";
				}
				print OUT "\tend if\n\n";
			}
			...
			if($variable_temp){
				print "\nVariable temperature not implemented in the psat and sigma properties.\n\n";
			}else{
				print OUT "\tcall get_coll$append_to_eq_names(nrates,K)\n";
			}
			...
			if($nmol_types_used==1){
				...
				print OUT "\t\tE(i,1) = K(i,1)*psat/$string1*exp((2.d0*sigma*monvol)/($string1*r))\n";
				print OUT "\t\tif (i .eq. 1) then\n";
				print OUT "\t\t\tE(i,1) = .5d0*E(i,1)\n";
				print OUT "\t\tend if\n";
				print OUT "\tend do\n\n";
				...
				if($lhs_tests){
					print OUT "\t\t\t\tij = clust_from_indices($nmol_ranges_ij_ind)\n";
					print OUT "\t\t\t\tif (lhs(i) .and. lhs(j) .and. lhs(ij)) then\n";
					print OUT "\t\t\t\t\tE(i,j) = K(i,j)*1.01325d5/$string1*&\n\t\t\t\t\t\t\t&exp((deltah(ij)-deltah(i)-deltah(j)-temperature*&\n\t\t\t\t\t\t\t&1.d-3*(deltas(ij)-deltas(i)-deltas(j)))*$string2)\n";
					print OUT "\t\t\t\telse\n";
					$str_temp = "\t";
				}
				print OUT "$str_temp\t\t\t\txmol = real(ij_ind(imol),kind=kind(1.d0))/real(sum(ij_ind),kind=kind(1.d0))\n";
				print OUT "$str_temp\t\t\t\tE(i,j) = K(i,j)*psat(imol)*xmol/$string1*&\n$str_temp\t\t\t\t\t\t&exp((2.d0*sigma*monvol(imol))/($string1*r))\n";
				if($lhs_tests){
					print OUT "\t\t\t\tend if\n";
				}
				print OUT "\t\t\t\tif (i .eq. j) then\n";
...
################################################################################
		...
		if($evaporation_coef_file_name ne ''){
            if(!$lsilent){
                print "\nEvaporation rate file: $evaporation_coef_file_name\n";
            }
	...
	################################################################################
		...
		if(($scale_evap_factor[0][0] == 0) && ($scale_evap_file_name eq '')){
			...
			if($scale_evap_file_name ne ''){
                if(!$lsilent){
                    print "\nEvaporation rate scaling file: $scale_evap_file_name\n";
                }
	...
	################################################################################
                    ...
                    if($l_no_delta_g_data[$iclus] || $l_no_delta_g_data[$jclus] || $l_no_delta_g_data[$kclus]){
                        die "Missing free energy or input evaporation rate data for $kclus_label -> $iclus_label + $jclus_label\n";
                    }
	...
	################################################################################
					...
					# If we are using a variable temperature, we don't have a hydrate distribution.
						...
						if(defined $evap_coef_scale[$iclus][$jclus]){
							$str_temp = sprintf('%.14e',$evap_coef_scale[$iclus][$jclus]/$kcal_per_mol_to_J);
							if($lfortran){
								$str_temp =~ s/e/d/;
							}
							$evap_coef[$iclus][$jclus]="$evap_coef[$iclus][$jclus]$string+$str_temp/temperature";
						}
						$temp_val = $kcal_per_mol_to_J/$boltz;
						...
						if($iclus == $jclus){
							if($lfortran){
								$evap_coef[$iclus][$jclus] = "0.5d0*$evap_coef[$iclus][$jclus]";
							}else{
	...
	################################################################################
				...
				if($levap_coef_in[$iclus][$jclus]){
					...
					if($lhydrate_average){
						if($lhas_hydr[$kclus]){
							if(defined $evap_coef_hydr[$iclus][$jclus][0][0]){
                                if($iclus == $jclus){
	...
	if($lfortran){
		print OUT "\nend subroutine get_evap$append_to_eq_names\n";
	}
}else{
	if(!$lfortran){
...
if($include_ion_terms && $lfortran && ($parameters_in_get_fcr_1 ne '')){
	print OUT "\n!-----------------------------------------------------------\n\n";
	print OUT "subroutine get_fcr$append_to_eq_names($parameters_in_get_fcr_1)\n\timplicit none\n";
	...
	print OUT "\nend subroutine get_fcr$append_to_eq_names\n";
}
...
if($lfortran && ($l_coll_factor_mcmc_n_i || $l_coll_factor_mcmc_n_g || ($l_evap_factor_mcmc && $missing_energies<$ncoef_evap))){
	print OUT "\n!-----------------------------------------------------------\n\n";
	print OUT "subroutine coefs_from_data$append_to_eq_names(coef)";
	...
	print OUT "\n\nend subroutine coefs_from_data$append_to_eq_names";
}
...
if(!$lfortran){
    ...
    $line = $line . "\t\t\telseif strcmpi(varargin{j},'difftol')\n";
	$line = $line . "\t\t\t\tdifftol = varargin{j+1};\n";
	...
	$line = $line . "\t\t\t\tn_fun = [n_fun find(strcmp(clust,varargin{j+1}{1}))];\n";
	...
	$line = $line . "\t\t\t\t\telse\n";
	$line = $line . "\t\t\t\t\t\tC_fun_str{n_fun(end)} = [C_fun_str{n_fun(end)}, '1e-6*c(', num2str(find(strcmp(clust,varargin{j+1}{k}))), ')'];\n";
	$line = $line . "\t\t\t\t\t\tC_fun_str_2{n_fun(end)} = [C_fun_str_2{n_fun(end)}, '1e-6*C(j,', num2str(find(strcmp(clust,varargin{j+1}{k}))), ')'];\n";
	$line = $line . "\t\t\t\t\t\tC_fun_str_3{n_fun(end)} = [C_fun_str_3{n_fun(end)}, '1e-6*C(j,', num2str(find(strcmp(clust,varargin{j+1}{k}))), ')'];\n";
	$line = $line . "\t\t\t\t\tend\n";
	...
	print DRIVER "\t\tA2 = A{2}(cellfun('isempty',regexp(A{1},'^(wall|coag)')));\n";
	print DRIVER "\t\tif any(~ismember(A2,clust)), warning(['Discarding erroneous cluster names in ',fname1]), end\n\n";

	print DRIVER "\t\tfor i=1:$max_cluster_number\n";
	print DRIVER "\t\t\tj = find(strcmp(A{2},clust{i}));\n";
	print DRIVER "\t\t\tdefined = 0;\n";
	print DRIVER "\t\t\tfor k=1:length(j)\n";
	print DRIVER "\t\t\t\tif (~C0_in && strncmp(A{1}(j(k)),'i',1))\n";
	...
	print DRIVER "\t\t\t\t\t\tA4 = regexp(A{4}{j(k)},'-','split');\n\t\t\t\t\t\tA4(cellfun('isempty',A4)) = [];\n";
    print DRIVER "\t\t\t\t\t\tif any(~ismember(A4,clust)), warning(['Discarding erroneous cluster names in ',fname1]), end\n";
	print DRIVER "\t\t\t\t\t\tfor i2=1:$max_cluster_number\n";
	print DRIVER "\t\t\t\t\t\t\tfor l=1:length(find(strcmp(A4,clust{i2})))\n";
	...
	print DRIVER "\t\tif min(min(C00)) < -c_min_th\n";
...
#	print DRIVER "\t\t\tform = nan;\n";
	...
	print DRIVER "\t\tif max(abs(Cf(1:$max_cluster_number)-C0(1:$max_cluster_number))./max(C0(1:$max_cluster_number),c_min_th)) < difftol\n\t\t\tconverged = 1;\n";
	print DRIVER "\t\telseif length(T)>2 && max(abs(Cf(1:$max_cluster_number)-C(floor(length(T)*.75),1:$max_cluster_number))./max(C(floor(length(T)*.75),1:$max_cluster_number),c_min_th)) < difftol\n\t\t\tconverged = 1;\n\t\tend\n";
	print DRIVER "\t\tif any(isfitted)\n";
	print DRIVER "\t\t\tfor j = 1:$max_cluster_number\n";
	print DRIVER "\t\t\t\tif isfitted(j)\n";
	print DRIVER "\t\t\t\t\tif abs(C_fit{j,1}-Cf(j)-sum(Cf(C_fit{j,2})))/C_fit{j,1} > difftol\n";
    ...
    if($include_generic_neg && $include_generic_pos){
		if($generic_positive_ions_corr ne ''){
			print DRIVER "\t\tif abs(sum(Cf(positives))-sum(Cf(negatives)))/max(Cf($n_generic_pos),c_min_th) > difftol\n";
			print DRIVER "\t\t\tif sum(Cf(negatives))-sum(Cf(positives))+Cf($n_generic_pos)>0\n";
			print DRIVER "\t\t\t\tCf($n_generic_pos) = sum(Cf(negatives))-sum(Cf(positives))+Cf($n_generic_pos);\n";
			print DRIVER "\t\t\telse\n";
			print DRIVER "\t\t\t\tCf($n_generic_neg) = sum(Cf(positives))-Cf($n_generic_pos)-sum(Cf(negatives))+Cf($n_generic_neg);\n";
			print DRIVER "\t\t\t\tCf($n_generic_pos) = 0;\n";
			print DRIVER "\t\t\tend\n";
			print DRIVER "\t\t\tconverged = 0;\n";
			print DRIVER "\t\tend\n\n";
		}elsif($generic_negative_ions_corr ne ''){
			print DRIVER "\t\tif abs(sum(Cf(positives))-sum(Cf(negatives)))/max(Cf($n_generic_neg),c_min_th) > difftol\n";
			print DRIVER "\t\t\tif sum(Cf(positives))-sum(Cf(negatives))+Cf($n_generic_neg)>0\n";
			print DRIVER "\t\t\t\tCf($n_generic_neg) = sum(Cf(positives))-sum(Cf(negatives))+Cf($n_generic_neg);\n";
			print DRIVER "\t\t\telse\n";
			print DRIVER "\t\t\t\tCf($n_generic_pos) = sum(Cf(negatives))-Cf($n_generic_neg)-sum(Cf(positives))+Cf($n_generic_pos);\n";
			print DRIVER "\t\t\t\tCf($n_generic_neg) = 0;\n";
			print DRIVER "\t\t\tend\n";
			print DRIVER "\t\t\tconverged = 0;\n";
			print DRIVER "\t\tend\n\n";
		}elsif($generic_positive_ions ne ''){
			print DRIVER "\t\tif abs(Cf($n_generic_pos)-sum(Cf(negatives)))/max(Cf($n_generic_pos),c_min_th) > difftol\n";
			print DRIVER "\t\t\tCf($n_generic_pos) = sum(Cf(negatives));\n";
			print DRIVER "\t\t\tconverged = 0;\n";
			print DRIVER "\t\tend\n\n";
		}elsif($generic_negative_ions ne ''){
			print DRIVER "\t\tif abs(Cf($n_generic_neg)-sum(Cf(positives)))/max(Cf($n_generic_neg),c_min_th) > difftol\n";
			print DRIVER "\t\t\tCf($n_generic_neg) = sum(Cf(positives));\n";
			print DRIVER "\t\t\tconverged = 0;\n";
			print DRIVER "\t\tend\n\n";
		}
	}
	...
	if(!$variable_temp){
		...
		print DRIVER "\t\t\tJ_out(i) = $j_function_name(C(i,:)*1e-6,temperature,K);\n";
	}
	print DRIVER "\t\tend\n";
	if($l_old_output){
		print DRIVER "\telse\n";
	...
	if($corr_negative_mol[0] !~ /^\s*$/ || $corr_positive_mol[0] !~ /^\s*$/){
		$temp_val += 1;
	}
...
# Variable 'form' is not printed to the driver, for now, since nobody is probably using it
#	$temp_val = ($temp_val-1)/2;
...
#	print DRIVER "\t\tform = form/2;\n\n";
	...
	if(!$l_old_output){
		...
		print DRIVER "\t\toutput_opt={labels_ch, clust_flux, J_out, flux, outflux_matrix, out, sources};\n";
		print DRIVER "\t\tvarargout=output_opt(1:nargout-5);\n";
		print DRIVER "\tend\n";
	}
	print DRIVER "end";
...
if(!$lsilent){
    print "\n###########################################################\n";
    if($lhydrate_average==1){
        print "Done printing the output files. RH = $rh %, P(water) = $pw Pa\n";
    }else{
        print "Done printing the output files.\n";
    }
    print "###########################################################\n";
}
...
## This also returns $lout, which is 1, 2 or 3 if the cluster got out, and 0 if it was brought back,
